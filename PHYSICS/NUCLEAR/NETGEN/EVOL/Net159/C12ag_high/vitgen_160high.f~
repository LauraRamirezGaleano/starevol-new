...
************************************************************************
      ...
      integer i,j,k,nreac,nre,ngrid,ngrid1,nvit,nj,iref
     ...
     &     ciso*1,ca*5,cca*8,ref*130,cref*130
...
c      parameter(nre = 180, nvit = 250)
 ...
 43   species(nsp) = 'NUTRI'
      ...
      open (unit = 30,file = 'err.log')
c..   read reference rates
      iref = 1
 61   read(30,'(a8)',end=62) cca
      if (cca.eq.' #  Date') then
         read (30,'(12x,a130)',end=62) ref(iref)
         iref = iref+1
      endif
      goto 61

 62   open (unit = 25,file = 'starevolnuc_out.par')
...
c 500        format(4(i4))
 ...
 25   nre = k
      write (*,21) nre
 21   format(' output : vitsub_',i3,'high.f')
      write (cseq,'(i3)') nre
      fileout = 'vitsub_' // cseq // 'high.f'
      open (unit = 10,file = trim(fileout))
 300  format (1x,f13.3)

      write(25,445)
 445  format(/,'nuclear network references:',/)
      do i = 1,iref-1
         write(25,446) i,ref(i)
      enddo
 446  format(2x,i3,2x,a130)
