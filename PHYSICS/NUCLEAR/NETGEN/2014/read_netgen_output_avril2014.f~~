...
c      parameter(ngrid=250,nre=10000) 
      parameter(ngrid=250,nre=55,nreac=167) 
...
c..   Index of the associated reaction within the STAREVOL v3.30 network
      data (sreacnum(i),i=1,nre) / 
     &     2, 3, 4, 5, 9, 7, 8, 10, 12, 13, 14, 17, 21, 22, 23,
     &     25, 24, 26, 27, 30, 31, 35, 36, 39, 48, 52, 51, 41, 45, 49,
     &     53, 58, 59, 67, 68, 72, 73, 74, 85, 87, 91, 95, 96, 97, 
     &     101, 103, 104, 107, 109, 112, 111, 117, 130, 133, 134/
...
c..   Read in starevol.par the species in the network and their number code
      open (unit = 99,file = 'starevol_3.30.par')
      read(99,40)
 40   format(32/)
      nsp = 0
...
c...  containing the reference for the adopted rate
      ...
      open(unit=16,file='updated_network_24avril2014_modified.refs')
      do i = 1,nre
         read(16,'(a172)')reference(i)
      enddo


      open(unit=98,file='nuclear_network_avril2014')
      do i = 1,nre
...
c..   introduces the new rates for the updated reactions
      ...
      open (unit = 10,file = 'vitsub_167d.f')
      open (unit = 20,file = 'vitsub_167c.f',status='old')
...
c     if iread = 0, start by reading the data file, then compute the rates
      ...
      if (iread.eq.0) then
         ...
         open(unit=10,file='updated_network_24avril2014.rates')
...
c..   Read the network that has been input to the netgen server
      if (iread.eq.0) then
         open (unit=15,file=
     &        'updated_network_24avril2014_modified.network')
         deut = 'H   2'
         prot = 'H   1'
         do i = 1,nre
