...
c...  containing the reference for the adopted rate
      ...
      open(unit=16,file='updated_Li7palphaBe7pgammaNACRE.refs')
      do i = 1,nre
         read(16,'(a172)')reference(i)
      enddo


      open(unit=98,file='nuclear_network_LiNACRE')
      do i = 1,nre
...
c..   introduces the new rates for the updated reactions
      ...
      open (unit = 10,file = 'vitsub_167f.f')
...
c     if iread = 0, start by reading the data file, then compute the rates
      ...
      if (iread.eq.0) then
         ...
         open(unit=10,file='Li7palphaBe7pgammaNACRE.rates')
...
c..   Read the network that has been input to the netgen server
      if (iread.eq.0) then
         open (unit=15,file=
     &        'Li7palphaBe7pgammaNACRE.network')
         deut = 'H   2'
         prot = 'H   1'
         do i = 1,nre
