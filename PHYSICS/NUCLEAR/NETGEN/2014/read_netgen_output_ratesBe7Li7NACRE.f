...
c.. Actual number of reactions = 2 (see file Li7palphaBe7pgammaNACRE.network)
c      parameter(ngrid=250,nre=10000) 
      parameter(ngrid=250,nre=8,nreac=167) 
...
c..   Index of the associated reaction within the STAREVOL v3.30 network
      data (sreacnum(i),i=1,nre) /17,14,10,12,15,18,20,24/
...
c...  containing the reference for the adopted rate
      ...
      open(unit=16,file='updtated_netwrokBe7Li7Nacre1.refs')
      do i = 1,nre
         read(16,'(a172)')reference(i)
      enddo


      open(unit=98,file='nuclear_network_Be7Li7NACRE')
      do i = 1,nre
...
c..   introduces the new rates for the updated reactions
      ...
      open (unit = 10,file = 'vitsub_167g.f')
...
c     if iread = 0, start by reading the data file, then compute the rates
      ...
      if (iread.eq.0) then
         ...
         open(unit=10,file='networkBe7Li7Nacre1.rates')
