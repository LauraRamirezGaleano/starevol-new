...
c..this program exercises the aprox19 network

c..declare
      character*40     hfile
      logical          lfile
      integer          i,j,nok,nbad,iind(19)
      double precision tstep,conserv,
     1                 tin,din,ein,xin(19),tout,dout,eout,xout(19),
     2                 ydum(19),abar,zbar
...
c..get the right hand sides, exact energy generation rate and so on 
        call aprox19(ttime(i),ydum,dydt_dum)
...
c..this file contains aprox19 network 
c..
c..routine aprox19 sets up the odes 
c..routine daprox19 sets up the dense aprox19 jacobian
c..routine aprox19rat generates the reaction rates for routine aprox19
c..routine aprox19tab generates the raw rates using table interpolation
c..routine screen_aprox19 applies screening corrections to the raw rates
c..routine faprox19 gets the flows from aprox19
c..routine init_aprox19 initializes the aprox19 network




      subroutine aprox19(tt,y,dydt)   
      include 'implno.dek'
      include 'const.dek'
      include 'burn_common.dek' 
      include 'network.dek'
...
c..this routine sets up the system of ode's for the aprox19 nuclear reactions.
c..this is an alpha chain + heavy ion network with (a,p)(p,g) links
c..
c..isotopes: h1,   he3,  he4,  c12,  n14,  o16,  ne20, mg24, si28, s32,
c..          ar36, ca40, ti44, cr48, fe52, fe54, ni56, n,    p
...
c..local variables
     ...
     1                 r1f54,r2f54,r3f54,r4f54,r5f54,r6f54,r7f54,r8f54,
     2                 ralf1,ralf2,
     3                 enuc,taud,taut,sum1,sum2,z,
     4                 zbarxx,ytot1,abar,zbar,
     5                 snudt,snudd,snuda,snudz

      double precision conv,sixth,c54
      parameter        (conv  = ev2erg*1.0d6*avo,
     1                  sixth = 1.0d0/6.0d0,
     2                  c54   = 56.0d0/54.0d0)     
...
c..get the weak rates here because they depend on the composition
...
c..fe52(n,g)fe53(n,g)fe54 dummy neutron links
      r1f54 = 0.0d0
      denom = ratdum(ir53gn) + y(ineut)*ratdum(ir53ng)
      if (denom .ne. 0.0 .and. btemp .gt. 1.5e9) then
       r1f54 = ratdum(ir54gn)*ratdum(ir53gn)/denom
      end if

      r2f54 = 0.0d0
      if (denom .ne. 0.0 .and. btemp .gt. 1.5e9) then
       r2f54 = ratdum(ir52ng)*ratdum(ir53ng)/denom
      end if



c..fe54(p,g)co55(p,g)ni56 dummy proton links
c..r4f54 tends to make the system quite stiff in nse
      r3f54 = 0.0d0
      denom = ratdum(ircogp) + y(iprot)*(ratdum(ircopg)+ratdum(ircopa))
      if (denom .ne. 0.0 .and. btemp .gt. 1.5e9) then
       r3f54 = ratdum(irfepg)*ratdum(ircopg)/denom
      end if

      r4f54 = 0.0d0
      if (denom .ne. 0.0 .and. btemp .gt. 1.5e9) then
       r4f54 = ratdum(irnigp)*ratdum(ircogp)/denom
      end if


c..fe52(a,p)co55(g,p)fe54 dummy links
      r5f54 = 0.0d0
      if (denom .ne. 0.0 .and. btemp .gt. 1.5e9) then
       r5f54 = ratdum(irfepg)*ratdum(ircopa)/denom
      end if


      r6f54 = 0.0d0
      if (denom .ne. 0.0 .and. btemp .gt. 1.5e9) then
       r6f54 = ratdum(irfeap)*ratdum(ircogp)/denom
      end if


c..fe52(a,p)co55(p,g)ni56 dummy links
      r7f54 = 0.0d0
      if (denom .ne. 0.0 .and. btemp .gt. 1.5e9) then
       r7f54 = ratdum(irfeap)*ratdum(ircopg)/denom
      end if

      r8f54 = 0.0d0
      if (denom .ne. 0.0 .and. btemp .gt. 1.5e9) then
       r8f54 = ratdum(irnigp)*ratdum(ircopa)/denom
      end if
...
c..p(n,g)h2(n,g)3h(p,g)he4   photodisintegrated n and p back to he4 dummy links
c..p(n,g)h2(p,g)he3(n,g)he4

      ralf1  = 0.0d0
      ralf2  = 0.0d0
      denom  = ratdum(irhegp)*ratdum(irdgn) + 
     1         y(ineut)*ratdum(irheng)*ratdum(irdgn) + 
     2         y(ineut)*y(iprot)*ratdum(irheng)*ratdum(irdpg)
      if (denom .ne. 0.0) then
       ralf1 = ratdum(irhegn)*ratdum(irhegp)*ratdum(irdgn)/denom
       ralf2 = ratdum(irheng)*ratdum(irdpg)*ratdum(irhng)/denom
      end if


c..he3(a,g)be7(p,g)8B(e+nu)8Be(2a)
c..beta limit he3+he4 by the 8B decay half life
      if (y(ihe4) .ne. 0.0) then
       ratdum(ir34) = min(ratdum(ir34),0.896d0/y(ihe4))
      end if


c..beta limit n14(p,g)o15(enu)o16  and o16(p,g)f17(e+nu)17o(p,a)n14
      if (y(ih1)  .ne. 0.0) then
       ratdum(irnpg) = min(ratdum(irnpg),5.68d-03/(y(ih1)*1.57d0))
       ratdum(iropg) = min(ratdum(iropg),0.0105d0/y(ih1))
      end if
...
c..hydrogen reactions
      dydt(ih1)  = -1.5d0 * y(ih1) * y(ih1) * ratdum(irpp)
     1            + y(ihe3) * y(ihe3) * ratdum(ir33) 
     2            - y(ihe3) * y(ihe4) * ratdum(ir34)      
     3            - 2.0d0 * y(ic12) * y(ih1) * ratdum(ircpg)
     4            - 2.0d0 * y(in14) * y(ih1) * ratdum(irnpg) 
     5            - 2.0d0 * y(io16) * y(ih1) * ratdum(iropg)
     6            - 3.0d0 * y(ih1) * ratdum(irpen)


c..he3 reactions
      dydt(ihe3) =  0.5d0 * y(ih1) * y(ih1) * ratdum(irpp)
     1            - y(ihe3) * y(ihe3) * ratdum(ir33)
     2            - y(ihe3) * y(ihe4) * ratdum(ir34)       
     3            + y(ih1) * ratdum(irpen)
...
c..photodisintegration reactions
      dydt(ihe4) =  dydt(ihe4)
     1            + y(ife54) * y(iprot) * y(iprot) * r5f54
     2            - y(ihe4) * y(ife52) * r6f54
     3            - y(ife52) * y(ihe4) * y(iprot) * r7f54
     4            + y(ini56) * y(iprot) * r8f54
     5            - y(ihe4) * ralf1           
     6            + y(ineut)**2 * y(iprot)**2 * ralf2

c..ppchain
      dydt(ihe4) =  dydt(ihe4)
     1            + 0.5d0 * y(ihe3) * y(ihe3) * ratdum(ir33)        
     2            + y(ihe3) * y(ihe4) * ratdum(ir34) 

c..cno cycles
      dydt(ihe4) =  dydt(ihe4)
     1            + y(in14) * y(ih1) * ratdum(ifa) * ratdum(irnpg)      
     2            + y(io16) * y(ih1) * ratdum(iropg) 
     3            - y(ihe4) * y(in14) * ratdum(irnag) * 1.5d0
...
c..c12 reactions   
     ...
     6            - y(ic12) * y(ih1) * ratdum(ircpg)          
     7            + y(in14) * y(ih1) * ratdum(ifa) * ratdum(irnpg)


c..n14 reactions
      dydt(in14) = y(ic12) * y(ih1) * ratdum(ircpg)
     1           - y(in14) * y(ih1) * ratdum(irnpg)
     2           + y(io16) * y(ih1) * ratdum(iropg)         
     3           - y(ihe4) * y(in14) * ratdum(irnag)
...
c..o16 reactions 
     ...
     6            + y(in14) * y(ih1) * ratdum(ifg) * ratdum(irnpg)       
     7            - y(io16) * y(ih1) * ratdum(iropg)
...
c..photodisintegration reactions
     ...
     1             + y(ife54) * r1f54
     2             - y(ife52) * y(ineut) * y(ineut) * r2f54      
     3             + y(ife54) * y(iprot) * y(iprot) * r5f54
     4             - y(ife52) * y(ihe4) * r6f54
     5             - y(ife52) * y(ihe4) * y(iprot) * r7f54  
     6             + y(ini56) * y(iprot) * r8f54



c..fe54 reactions
      dydt(ife54) = -y(ife54) * r1f54
     1             + y(ife52) * y(ineut) * y(ineut) * r2f54      
     2             - y(ife54) * y(iprot) * y(iprot) * r3f54
     3             + y(ini56) * r4f54
     4             - y(ife54) * y(iprot) * y(iprot) * r5f54
     5             + y(ife52) * y(ihe4) * r6f54           
     6             + c54 * y(ini56) * ratdum(irn56ec) 
...
c..ni56 reactions
     ...
     4             - y(ini56) * ratdum(irn56ec) 

c..photodisintegration reactions
      dydt(ini56) = dydt(ini56) 
     1             + y(ife54) * y(iprot) * y(iprot) * r3f54
     2             - y(ini56) * r4f54 
     3             + y(ife52) * y(ihe4)* y(iprot) * r7f54
     4             - y(ini56) * y(iprot) * r8f54
...
c..photodisintegration neutrons
      dydt(ineut) =  2.0d0 * y(ife54) * r1f54        
     1             - 2.0d0 * y(ife52) * y(ineut) * y(ineut) * r2f54 
     2              + 2.0d0 * y(ihe4) * ralf1
     3              - y(ineut)**2 * y(iprot)**2 * ralf2
     4              + y(iprot) * ratdum(irpen)
     5              - y(ineut) * ratdum(irnep)             



c..photodisintegration protons
      dydt(iprot) = -2.0d0 * y(ife54) * y(iprot) * y(iprot) * r3f54
     1             + 2.0d0 * y(ini56) * r4f54           
     2             - 2.0d0 * y(ife54) * y(iprot) * y(iprot) * r5f54
     3             + 2.0d0 * y(ife52) * y(ihe4) * r6f54
     4             + 2.0d0 * y(ihe4) * ralf1
     5             - y(ineut)**2 * y(iprot)**2 * ralf2
     6             - y(iprot) * ratdum(irpen)                
     7             + y(ineut) * ratdum(irnep)
...
c..temperature equation that is self-consistent with an eos 
     ...
     1                   - sum1 - sum2)
      ...
      subroutine daprox19(tt,y,dfdy,nlog,nphys)   
      include 'implno.dek'
      include 'const.dek'
      include 'burn_common.dek'
      include 'network.dek'
...
c..this routine sets up the dense aprox19 jacobian
c.. 
c..declare the pass
      integer          nlog,nphys
...
c..local variables
      integer          i,j
      double precision denom,denomdt,denomdd,
     1                 r1,r1dt,r1dd,s1,s1dt,s1dd,t1,t1dt,t1dd,
     2                 u1,u1dt,u1dd,v1,v1dt,v1dd,w1,w1dt,w1dd,
     3                 x1,x1dt,x1dd,y1,y1dt,y1dd,zz,sum1,sum2

      double precision r1f54,r1f54dn,r1f54dt,r1f54dd,
     1                 r2f54,r2f54dn,r2f54dt,r2f54dd,
     2                 r3f54,r3f54dp,r3f54dt,r3f54dd,
     3                 r4f54,r4f54dp,r4f54dt,r4f54dd,
     4                 r5f54,r5f54dp,r5f54dt,r5f54dd,
     5                 r6f54,r6f54dp,r6f54dt,r6f54dd,
     6                 r7f54,r7f54dp,r7f54dt,r7f54dd,
     7                 r8f54,r8f54dp,r8f54dt,r8f54dd,
     8                 ralf1,ralf1dn,ralf1dp,ralf1dt,ralf1dd,
     9                 ralf2,ralf2dn,ralf2dp,ralf2dt,ralf2dd,
     &                 xx,dir34da,dirnpgdp,diropgdp
...
c..fe52(n,g)fe53(n,g)fe54 dummy neutron links
      ...
      if (denom .ne. 0.0 .and. btemp .gt. 1.5e9) then
       ...
       r1f54   = ratdum(ir54gn)*ratdum(ir53gn)*zz
       r1f54dn = -r1f54*zz * ratdum(ir53ng)
       r1f54dt = dratdumdt(ir54gn)*ratdum(ir53gn)*zz
     1          + ratdum(ir54gn)*dratdumdt(ir53gn)*zz
     2          - r1f54*zz*denomdt
       r1f54dd = dratdumdd(ir54gn)*ratdum(ir53gn)*zz
     1          + ratdum(ir54gn)*dratdumdd(ir53gn)*zz
     2          - r1f54*zz*denomdd

       r2f54   = ratdum(ir52ng)*ratdum(ir53ng)*zz
       r2f54dn = -r2f54*zz * ratdum(ir53ng)
       r2f54dt = dratdumdt(ir52ng)*ratdum(ir53ng)*zz
     1          + ratdum(ir52ng)*dratdumdt(ir53ng)*zz
     2          - r2f54*zz*denomdt
       r2f54dd = dratdumdd(ir52ng)*ratdum(ir53ng)*zz
     1          + ratdum(ir52ng)*dratdumdd(ir53ng)*zz
     2          - r2f54*zz*denomdd
...
c..fe54(p,g)co55(p,g)ni56 dummy proton links r3f54 r4f54
c..fe52(a,p)co55(g,p)fe54 dummy proton links r5f54 r6f54
c..fe52(a,p)co55(p,g)ni56 dummy proton links r7f54 r8f54
c..r4f54 tends to make the system quite stiff in nse
      r3f54   = 0.0d0
      r3f54dp = 0.0d0
      r3f54dt = 0.0d0
     ...
     1          + y(iprot)*(dratdumdt(ircopg) + dratdumdt(ircopa))
       denomdd = dratdumdd(ircogp) 
     1          + y(iprot)*(dratdumdd(ircopg) + dratdumdd(ircopa))

       zz      = 1.0d0/denom

       r3f54   = ratdum(irfepg) * ratdum(ircopg) * zz
       r3f54dp = -r3f54 * zz * (ratdum(ircopg) + ratdum(ircopa)) 
       r3f54dt =   dratdumdt(irfepg) * ratdum(ircopg) * zz
     1           + ratdum(irfepg) * dratdumdt(ircopg) * zz
     2           - r3f54*zz*denomdt
       r3f54dd =   dratdumdd(irfepg) * ratdum(ircopg) * zz
     1           + ratdum(irfepg) * dratdumdd(ircopg) * zz
     2           - r3f54*zz*denomdd

       r4f54   = ratdum(irnigp) * ratdum(ircogp) * zz
       r4f54dp = -r4f54*zz * (ratdum(ircopg)+ratdum(ircopa)) 
       r4f54dt =   dratdumdt(irnigp) * ratdum(ircogp) * zz
     1           + ratdum(irnigp) * dratdumdt(ircogp) * zz
     2           - r4f54*zz*denomdt
       r4f54dd =   dratdumdd(irnigp) * ratdum(ircogp) * zz
     1           + ratdum(irnigp) * dratdumdd(ircogp) * zz
     2           - r4f54*zz*denomdd

       r5f54   = ratdum(irfepg) * ratdum(ircopa) * zz
       r5f54dp = -r5f54 * zz * (ratdum(ircopg)+ratdum(ircopa)) 
       r5f54dt = dratdumdt(irfepg) * ratdum(ircopa) * zz
     1           + ratdum(irfepg) * dratdumdt(ircopa) * zz
     2           - r5f54 * zz * denomdt
       r5f54dd = dratdumdd(irfepg) * ratdum(ircopa) * zz
     1           + ratdum(irfepg) * dratdumdd(ircopa) * zz
     2           - r5f54 * zz * denomdd

       r6f54   = ratdum(irfeap) * ratdum(ircogp) * zz
       r6f54dp = -r6f54 * zz * (ratdum(ircopg)+ratdum(ircopa)) 
       r6f54dt = dratdumdt(irfeap) * ratdum(ircogp) * zz
     1           + ratdum(irfeap) * dratdumdt(ircogp) * zz
     2           - r6f54 * zz * denomdt
       r6f54dd = dratdumdd(irfeap) * ratdum(ircogp) * zz
     1           + ratdum(irfeap) * dratdumdd(ircogp) * zz
     2           - r6f54 * zz * denomdd

       r7f54   = ratdum(irfeap) * ratdum(ircopg) * zz
       r7f54dp = -r7f54 * zz * (ratdum(ircopg)+ratdum(ircopa))
       r7f54dt = dratdumdt(irfeap) * ratdum(ircopg) * zz
     1           + ratdum(irfeap) * dratdumdt(ircopg) * zz
     2           - r7f54 * zz * denomdt
       r7f54dd = dratdumdd(irfeap) * ratdum(ircopg) * zz
     1           + ratdum(irfeap) * dratdumdd(ircopg) * zz
     2           - r7f54 * zz * denomdd 

       r8f54   = ratdum(irnigp) * ratdum(ircopa) * zz
       r8f54dp = -r8f54 * zz * (ratdum(ircopg)+ratdum(ircopa)) 
       r8f54dt = dratdumdt(irnigp) * ratdum(ircopa) * zz
     1           + ratdum(irnigp) * dratdumdt(ircopa) * zz
     2           - r8f54 * zz * denomdt
       r8f54dd = dratdumdd(irnigp) * ratdum(ircopa) * zz
     1           + ratdum(irnigp) * dratdumdd(ircopa) * zz
     2           - r8f54 * zz * denomdd
...
c..p(n,g)h2(p,g)he3(n,g)he4
     ...
     1          + ratdum(irhegp)*dratdumdt(irdgn)  
     2          +  y(ineut) * (dratdumdt(irheng)*ratdum(irdgn) 
     3                       + ratdum(irheng)*dratdumdt(irdgn)) 
     4          +  y(ineut)*y(iprot) * (dratdumdt(irheng)*ratdum(irdpg)
     5                                + ratdum(irheng)*dratdumdt(irdpg))

       denomdd  = dratdumdd(irhegp)*ratdum(irdgn) 
     1          + ratdum(irhegp)*dratdumdd(irdgn)  
     2          +  y(ineut) * (dratdumdd(irheng)*ratdum(irdgn) 
     3                       + ratdum(irheng)*dratdumdd(irdgn)) 
     4          +  y(ineut)*y(iprot) * (dratdumdd(irheng)*ratdum(irdpg)
     5                                + ratdum(irheng)*dratdumdd(irdpg))

       zz = 1.0d0/denom  

       ralf1   = ratdum(irhegn)*ratdum(irhegp)*ratdum(irdgn)*zz
       ralf1dn = -ralf1*zz * (ratdum(irheng)*ratdum(irdgn) + 
     1                        y(iprot)*ratdum(irheng)*ratdum(irdpg))
       ralf1dp = -ralf1*zz * y(ineut)*ratdum(irheng)*ratdum(irdpg)
       ralf1dt =   dratdumdt(irhegn)*ratdum(irhegp)*ratdum(irdgn)*zz
     1           + ratdum(irhegn)*dratdumdt(irhegp)*ratdum(irdgn)*zz
     2           + ratdum(irhegn)*ratdum(irhegp)*dratdumdt(irdgn)*zz
     3           - ralf1*zz*denomdt
       ralf1dd =   dratdumdd(irhegn)*ratdum(irhegp)*ratdum(irdgn)*zz
     1           + ratdum(irhegn)*dratdumdd(irhegp)*ratdum(irdgn)*zz
     2           + ratdum(irhegn)*ratdum(irhegp)*dratdumdd(irdgn)*zz
     3           - ralf1*zz*denomdt


       ralf2   = ratdum(irheng)*ratdum(irdpg)*ratdum(irhng)*zz
       ralf2dn = -ralf2*zz * (ratdum(irheng)*ratdum(irdgn) + 
     1                        y(iprot)*ratdum(irheng)*ratdum(irdpg))
       ralf2dp = -ralf2*zz * y(ineut)*ratdum(irheng)*ratdum(irdpg)
       ralf2dt =   dratdumdt(irheng)*ratdum(irdpg)*ratdum(irhng)*zz
     1           + ratdum(irheng)*dratdumdt(irdpg)*ratdum(irhng)*zz
     2           + ratdum(irheng)*ratdum(irdpg)*dratdumdt(irhng)*zz
     3           - ralf2*zz*denomdt
       ralf2dd =   dratdumdd(irheng)*ratdum(irdpg)*ratdum(irhng)*zz
     1           + ratdum(irheng)*dratdumdd(irdpg)*ratdum(irhng)*zz
     2           + ratdum(irheng)*ratdum(irdpg)*dratdumdd(irhng)*zz
     3           - ralf2*zz*denomdd
...
c..beta limit he3+he4 by the 8B decay half life
      if (y(ihe4) .ne. 0.0) then
       xx            = 0.896d0/y(ihe4)
       ratdum(ir34)  = min(ratdum(ir34),xx)
       if (ratdum(ir34) .eq. xx) then
        dir34da         = -xx/y(ihe4)
        dratdumdt(ir34) = 0.0d0
        dratdumdd(ir34) = 0.0d0
       else
        dir34da = 0.0d0
       endif
      endif
...
c..h1 jacobian elements
      dfdy(ih1,ih1)  = -1.5d0 * y(ih1) * ratdum(irpp)
     1                - 2.0d0 * y(ic12) * ratdum(ircpg)
     2                - 2.0d0 * y(in14) * ratdum(irnpg) 
     3                - 2.0d0 * y(in14) * y(ih1) * dirnpgdp 
     4                - 2.0d0 * y(io16) * ratdum(iropg)
     5                - 2.0d0 * y(io16) * y(ih1) * diropgdp
     6                - 3.0d0 * ratdum(irpen)

      dfdy(ih1,ihe3) = 2.0d0 * y(ihe3) * ratdum(ir33) 
     1               - y(ihe4) * ratdum(ir34)      

      dfdy(ih1,ihe4) = -y(ihe3) * ratdum(ir34)      
     1                - y(ihe3) * y(ihe4) * dir34da
...
c..he3 jacobian elements
      dfdy(ihe3,ih1)  =  0.5d0 * y(ih1) * ratdum(irpp)
     1                 + ratdum(irpen)
...
c..he4 jacobian elements
     ...
     1                + y(in14) * y(ih1) * ratdum(ifa) * dirnpgdp      
     2                + y(io16) * ratdum(iropg) 
     3                + y(io16) * y(ih1) * diropgdp 
     ...
     8                   - y(ife52) * ratdum(irfeap) * (1.0d0-y1)
      ...
      dfdy(ihe4,ihe4)  = dfdy(ihe4,ihe4)
     1                   - y(ife52) * r6f54
     2                   - y(ife52) * y(iprot) * r7f54
     3                   - ralf1           

      dfdy(ihe4,ihe4)  = dfdy(ihe4,ihe4)
     1                 + y(ihe3) * ratdum(ir34) 
     2                 + y(ihe3) * y(ihe4) * dir34da
     3                 - y(in14) * ratdum(irnag) * 1.5d0
     ...
     3                   + x1 * ratdum(irfegp) 
     4                   - y(ihe4) * r6f54
     5                   - y(ihe4) * y(iprot) * r7f54

      dfdy(ihe4,ife54) =   y(iprot) * y(iprot) * r5f54  
     ...
     2                   + y(iprot) * r8f54
      ...
      dfdy(ihe4,ineut) = -y(ihe4) * ralf1dn           
     1                  + 2.0d0 * y(ineut) * y(iprot)**2 * ralf2
     2                  + y(ineut)**2 * y(iprot)**2 * ralf2dn

      dfdy(ihe4,iprot) =   2.0d0 * y(ife54) * y(iprot) * r5f54  
     1                   + y(ife54) * y(iprot) * y(iprot) * r5f54dp  
     2                   - y(ihe4) * y(ife52) * r6f54dp
     3                   - y(ife52) * y(ihe4) * r7f54
     4                   - y(ife52) * y(ihe4) * y(iprot) * r7f54dp
     5                   + y(ini56) * r8f54
     6                   + y(ini56) * y(iprot) * r8f54dp
     7                   - y(ihe4) * ralf1dp
     8                   + 2.0d0 * y(ineut)**2 * y(iprot) * ralf2
     9                   + y(ineut)**2 * y(iprot)**2 * ralf2dp



c..c12 jacobian elements
      dfdy(ic12,ih1)  = -y(ic12) * ratdum(ircpg)          
     1                 + y(in14) * ratdum(ifa) * ratdum(irnpg)
     2                 + y(in14) * y(ih1) * ratdum(ifa) * dirnpgdp
...
c..n14 jacobian elements
      dfdy(in14,ih1)  = y(ic12) * ratdum(ircpg)
     1                - y(in14) * ratdum(irnpg)
     2                - y(in14) * y(ih1) * dirnpgdp
     3                + y(io16) * ratdum(iropg)         
     4                + y(io16) * y(ih1) * diropgdp
...
c..o16 jacobian elements
     ...
     1               + y(in14) * y(ih1) * ratdum(ifg) * dirnpgdp
     2               - y(io16) * ratdum(iropg)
     3               - y(io16) * y(ih1) * diropgdp
...
c..fe52 jacobian elements
     ...
     4                    - y(ineut) * y(ineut) * r2f54      
     5                    - y(ihe4) * r6f54
     6                    - y(ihe4) * y(iprot) * r7f54  


      dfdy(ife52,ife54) = r1f54
     1                  + y(iprot) * y(iprot) * r5f54

      dfdy(ife52,ini56) = ratdum(irniga)
     1                  + y1 * ratdum(irnigp)
     2                  + y(iprot) * r8f54

      dfdy(ife52,ineut) =  y(ife54) * r1f54dn 
     1                   - 2.0d0 * y(ife52) * y(ineut) * r2f54      
     2                   - y(ife52) * y(ineut) * y(ineut) * r2f54dn

      dfdy(ife52,iprot) = 2.0d0 * y(ife54) * y(iprot) * r5f54
     1                  + y(ife54) * y(iprot) * y(iprot) * r5f54dp  
     2                  - y(ihe4) * y(ife52) * r6f54dp
     3                  - y(ife52) * y(ihe4) * r7f54  
     4                  - y(ife52) * y(ihe4) * y(iprot) * r7f54dp  
...
c..fe54 jacobian elements
      dfdy(ife54,ihe4)  = y(ife52) * r6f54

      dfdy(ife54,ife52) = y(ineut) * y(ineut) * r2f54      
     1                  + y(ihe4) * r6f54

      dfdy(ife54,ife54) = -r1f54
     1                   - y(iprot) * y(iprot) * r3f54
     2                   - y(iprot) * y(iprot) * r5f54

      dfdy(ife54,ini56) = r4f54
     1                  + c54 * ratdum(irn56ec) 

      dfdy(ife54,ineut) =  -y(ife54) * r1f54dn 
     1                   + 2.0d0 * y(ife52) * y(ineut) * r2f54    
     2                   + y(ife52) * y(ineut) * y(ineut) * r2f54dn  


      dfdy(ife54,iprot) = -2.0d0 * y(ife54) * y(iprot) * r3f54
     1                   - y(ife54) * y(iprot) * y(iprot) * r3f54dp
     2                   + y(ini56) * r4f54dp
     3                   - 2.0d0 * y(ife54) * y(iprot) * r5f54
     4                   - y(ife54) * y(iprot) * y(iprot) * r5f54dp  
     5                   + y(ihe4) * y(ife52) * r6f54dp
...
c..ni56 jacobian elements
     ...
     2                  + y(ihe4)* y(iprot) * r7f54

      dfdy(ini56,ife54) = y(iprot) * y(iprot) * r3f54
     ...
     4                   - ratdum(irn56ec) 
      ...
      dfdy(ini56,iprot) = 2.0d0 * y(ife54) * y(iprot) * r3f54
     1                  + y(ife54) * y(iprot) * y(iprot) * r3f54dp
     2                  - y(ini56) * r4f54dp
     3                  + y(ife52) * y(ihe4)* r7f54
     4                  + y(ife52) * y(ihe4)* y(iprot) * r7f54dp
...
c..photodisintegration neutrons jacobian elements
      dfdy(ineut,ihe4)  = 2.0d0 * ralf1

      dfdy(ineut,ife52) = -2.0d0 * y(ineut) * y(ineut) * r2f54  

      dfdy(ineut,ife54) =  2.0d0 * r1f54        

      dfdy(ineut,ineut) =  2.0d0 * y(ife54) * r1f54dn  
     1                   - 4.0d0 * y(ife52) * y(ineut) * r2f54 
     2                   - 2.0d0 * y(ife52) * y(ineut)*y(ineut)*r2f54dn
     3                   + 2.0d0 * y(ihe4) * ralf1dn
     4                   - 2.0d0 * y(ineut) * y(iprot)**2 * ralf2
     5                   - y(ineut)**2 * y(iprot)**2 * ralf2dn
     6                   - ratdum(irnep)             

      dfdy(ineut,iprot) = 2.0d0 * y(ihe4) * ralf1dp 
     1                  - 2.0d0 * y(ineut)**2 * y(iprot) * ralf2
     2                  - y(ineut)**2 * y(iprot)**2 * ralf2dp
     3                  + ratdum(irpen)


c..photodisintegration protons jacobian elements
      dfdy(iprot,ihe4)  = 2.0d0 * y(ife52) * r6f54
     1                  + 2.0d0 * ralf1

      dfdy(iprot,ife52) = 2.0d0 * y(ihe4) * r6f54

      dfdy(iprot,ife54) = -2.0d0 * y(iprot) * y(iprot) * r3f54
     1                   - 2.0d0 * y(iprot) * y(iprot) * r5f54

      dfdy(iprot,ini56) = 2.0d0 * r4f54           

      dfdy(iprot,ineut) = 2.0d0 * y(ihe4) * ralf1dn
     1                  - 2.0d0 * y(ineut) * y(iprot)**2 * ralf2
     2                  - y(ineut)**2 * y(iprot)**2 * ralf2dn
     3                  + ratdum(irnep)


      dfdy(iprot,iprot) = -4.0d0 * y(ife54) * y(iprot) * r3f54
     1                   - 2.0d0 * y(ife54) * y(iprot)*y(iprot)*r3f54dp
     2                   + 2.0d0 * y(ini56) * r4f54dp
     3                   - 4.0d0 * y(ife54) * y(iprot) * r5f54
     4                   - 2.0d0 * y(ife54) * y(iprot)*y(iprot)*r5f54dp  
     5                   + 2.0d0 * y(ihe4) * y(ife52) * r6f54dp
     6                   + 2.0d0 * y(ihe4) * ralf1dp
     7                   - 2.0d0 * y(ineut)**2 * y(iprot) * ralf2
     8                   - y(ineut)**2 * y(iprot)**2 * ralf2dp
     9                   - ratdum(irpen)                
...
c..hydrogen reactions
      dfdy(ih1,itemp)  = -1.5d0 * y(ih1) * y(ih1) * dratdumdt(irpp)
     1            + y(ihe3) * y(ihe3) * dratdumdt(ir33) 
     2            - y(ihe3) * y(ihe4) * dratdumdt(ir34)      
     3            - 2.0d0 * y(ic12) * y(ih1) * dratdumdt(ircpg)
     4            - 2.0d0 * y(in14) * y(ih1) * dratdumdt(irnpg) 
     5            - 2.0d0 * y(io16) * y(ih1) * dratdumdt(iropg)
     6            - 3.0d0 * y(ih1) * dratdumdt(irpen)


c..he3 reactions
      dfdy(ihe3,itemp) =  0.5d0 * y(ih1) * y(ih1) * dratdumdt(irpp)
     1            - y(ihe3) * y(ihe3) * dratdumdt(ir33)
     2            - y(ihe3) * y(ihe4) * dratdumdt(ir34)       
     3            + y(ih1) * dratdumdt(irpen)
...
c..photodisintegration reactions
      dfdy(ihe4,itemp) =  dfdy(ihe4,itemp)
     1            + y(ife54) * y(iprot) * y(iprot) * r5f54dt
     2            - y(ihe4) * y(ife52) * r6f54dt
     3            - y(ife52) * y(ihe4) * y(iprot) * r7f54dt
     4            + y(ini56) * y(iprot) * r8f54dt
     5            - y(ihe4) * ralf1dt           
     6            + y(ineut)**2 * y(iprot)**2 * ralf2dt

c..ppchain
      dfdy(ihe4,itemp) =  dfdy(ihe4,itemp)
     1            + 0.5d0 * y(ihe3) * y(ihe3) * dratdumdt(ir33)        
     2            + y(ihe3) * y(ihe4) * dratdumdt(ir34) 

c..cno cycles
      dfdy(ihe4,itemp) =  dfdy(ihe4,itemp)
     1            + y(in14) * y(ih1) * dratdumdt(ifa) * ratdum(irnpg)      
     1            + y(in14) * y(ih1) * ratdum(ifa) * dratdumdt(irnpg)      
     2            + y(io16) * y(ih1) * dratdumdt(iropg) 
     3            - y(ihe4) * y(in14) * dratdumdt(irnag) * 1.5d0
...
c..c12 reactions   
     ...
     6            - y(ic12) * y(ih1) * dratdumdt(ircpg)          
     7            + y(in14) * y(ih1) * dratdumdt(ifa) * ratdum(irnpg)
     7            + y(in14) * y(ih1) * ratdum(ifa) * dratdumdt(irnpg)


c..n14 reactions
      dfdy(in14,itemp) = y(ic12) * y(ih1) * dratdumdt(ircpg)
     1           - y(in14) * y(ih1) * dratdumdt(irnpg)
     2           + y(io16) * y(ih1) * dratdumdt(iropg)         
     3           - y(ihe4) * y(in14) * dratdumdt(irnag)


c..o16 reactions 
      dfdy(io16,itemp) = -y(ic12) * y(io16) * dratdumdt(ir1216)       
     1            - y(io16) * y(io16) * dratdumdt(ir1616) 
     2            + y(ic12) * y(ihe4) * dratdumdt(ircag)
     3            - y(io16) * y(ihe4) * dratdumdt(iroag)
     4            - y(io16) * dratdumdt(iroga)             
     5            + y(ine20) * dratdumdt(irnega)
     6            + y(in14) * y(ih1) * dratdumdt(ifg) * ratdum(irnpg)       
     6            + y(in14) * y(ih1) * ratdum(ifg) * dratdumdt(irnpg)       
     7            - y(io16) * y(ih1) * dratdumdt(iropg)

c..ne20 reactions 
      dfdy(ine20,itemp) =  0.5d0 * y(ic12) * y(ic12) * dratdumdt(ir1212)      
     1             + y(io16) * y(ihe4) * dratdumdt(iroag)       
     2             - y(ine20) * y(ihe4) * dratdumdt(irneag)
     3             - y(ine20) * dratdumdt(irnega)
     4             + y(img24) * dratdumdt(irmgga)            
     5             + y(in14) * y(ihe4) * dratdumdt(irnag)
...
c..photodisintegration reactions
      dfdy(ife52,itemp) = dfdy(ife52,itemp) 
     1             + y(ife54) * r1f54dt
     2             - y(ife52) * y(ineut) * y(ineut) * r2f54dt      
     3             + y(ife54) * y(iprot) * y(iprot) * r5f54dt
     4             - y(ife52) * y(ihe4) * r6f54dt
     5             - y(ife52) * y(ihe4) * y(iprot) * r7f54dt  
     6             + y(ini56) * y(iprot) * r8f54dt



c..fe54 reactions
      dfdy(ife54,itemp) = -y(ife54) * r1f54dt
     1             + y(ife52) * y(ineut) * y(ineut) * r2f54dt      
     2             - y(ife54) * y(iprot) * y(iprot) * r3f54dt
     3             + y(ini56) * r4f54dt
     4             - y(ife54) * y(iprot) * y(iprot) * r5f54dt
     5             + y(ife52) * y(ihe4) * r6f54dt           
     6             + c54 * y(ini56) * dratdumdt(irn56ec) 
...
c..ni56 reactions
     ...
     4             - y(ini56) * dratdumdt(irn56ec) 



c..photodisintegration reactions
      dfdy(ini56,itemp) = dfdy(ini56,itemp) 
     1             + y(ife54) * y(iprot) * y(iprot) * r3f54dt
     2             - y(ini56) * r4f54dt 
     3             + y(ife52) * y(ihe4)* y(iprot) * r7f54dt
     4             - y(ini56) * y(iprot) * r8f54dt
...
c..photodisintegration neutrons
      dfdy(ineut,itemp) =  2.0d0 * y(ife54) * r1f54dt        
     1             - 2.0d0 * y(ife52) * y(ineut) * y(ineut) * r2f54dt 
     2              + 2.0d0 * y(ihe4) * ralf1dt
     3              - y(ineut)**2 * y(iprot)**2 * ralf2dt
     4              + y(iprot) * dratdumdt(irpen)
     5              - y(ineut) * dratdumdt(irnep)             



c..photodisintegration protons
      dfdy(iprot,itemp) = -2.0d0 * y(ife54) * y(iprot)*y(iprot)*r3f54dt
     1             + 2.0d0 * y(ini56) * r4f54dt           
     2             - 2.0d0 * y(ife54) * y(iprot) * y(iprot) * r5f54dt
     3             + 2.0d0 * y(ife52) * y(ihe4) * r6f54dt
     4             + 2.0d0 * y(ihe4) * ralf1dt
     5             - y(ineut)**2 * y(iprot)**2 * ralf2dt
     6             - y(iprot) * dratdumdt(irpen)                
     7             + y(ineut) * dratdumdt(irnep)
...
c..hydrogen reactions
      dfdy(ih1,iden)  = -1.5d0 * y(ih1) * y(ih1) * dratdumdd(irpp)
     1            + y(ihe3) * y(ihe3) * dratdumdd(ir33) 
     2            - y(ihe3) * y(ihe4) * dratdumdd(ir34)      
     3            - 2.0d0 * y(ic12) * y(ih1) * dratdumdd(ircpg)
     4            - 2.0d0 * y(in14) * y(ih1) * dratdumdd(irnpg) 
     5            - 2.0d0 * y(io16) * y(ih1) * dratdumdd(iropg)
     6            - 3.0d0 * y(ih1) * dratdumdd(irpen)


c..he3 reactions
      dfdy(ihe3,iden) =  0.5d0 * y(ih1) * y(ih1) * dratdumdd(irpp)
     1            - y(ihe3) * y(ihe3) * dratdumdd(ir33)
     2            - y(ihe3) * y(ihe4) * dratdumdd(ir34)       
     3            + y(ih1) * dratdumdd(irpen)
...
c..photodisintegration reactions
      dfdy(ihe4,iden) =  dfdy(ihe4,iden)
     1            + y(ife54) * y(iprot) * y(iprot) * r5f54dd
     2            - y(ihe4) * y(ife52) * r6f54dd
     3            - y(ife52) * y(ihe4) * y(iprot) * r7f54dd
     4            + y(ini56) * y(iprot) * r8f54dd
     5            - y(ihe4) * ralf1dd           
     6            + y(ineut)**2 * y(iprot)**2 * ralf2dd

c..ppchain
      dfdy(ihe4,iden) =  dfdy(ihe4,iden)
     1            + 0.5d0 * y(ihe3) * y(ihe3) * dratdumdd(ir33)        
     2            + y(ihe3) * y(ihe4) * dratdumdd(ir34) 

c..cno cycles
      dfdy(ihe4,iden) =  dfdy(ihe4,iden)
     1            + y(in14) * y(ih1) * dratdumdd(ifa) * ratdum(irnpg)      
     1            + y(in14) * y(ih1) * ratdum(ifa) * dratdumdd(irnpg)      
     2            + y(io16) * y(ih1) * dratdumdd(iropg) 
     3            - y(ihe4) * y(in14) * dratdumdd(irnag) * 1.5d0
...
c..c12 reactions   
     ...
     6            - y(ic12) * y(ih1) * dratdumdd(ircpg)          
     7            + y(in14) * y(ih1) * dratdumdd(ifa) * ratdum(irnpg)
     7            + y(in14) * y(ih1) * ratdum(ifa) * dratdumdd(irnpg)


c..n14 reactions
      dfdy(in14,iden) = y(ic12) * y(ih1) * dratdumdd(ircpg)
     1           - y(in14) * y(ih1) * dratdumdd(irnpg)
     2           + y(io16) * y(ih1) * dratdumdd(iropg)         
     3           - y(ihe4) * y(in14) * dratdumdd(irnag)


c..o16 reactions 
      dfdy(io16,iden) = -y(ic12) * y(io16) * dratdumdd(ir1216)       
     1            - y(io16) * y(io16) * dratdumdd(ir1616) 
     2            + y(ic12) * y(ihe4) * dratdumdd(ircag)
     3            - y(io16) * y(ihe4) * dratdumdd(iroag)
     4            - y(io16) * dratdumdd(iroga)             
     5            + y(ine20) * dratdumdd(irnega)
     6            + y(in14) * y(ih1) * dratdumdd(ifg) * ratdum(irnpg)       
     6            + y(in14) * y(ih1) * ratdum(ifg) * dratdumdd(irnpg)       
     7            - y(io16) * y(ih1) * dratdumdd(iropg)

c..ne20 reactions 
      dfdy(ine20,iden) =  0.5d0 * y(ic12) * y(ic12) * dratdumdd(ir1212)      
     1             + y(io16) * y(ihe4) * dratdumdd(iroag)       
     2             - y(ine20) * y(ihe4) * dratdumdd(irneag)
     3             - y(ine20) * dratdumdd(irnega)
     4             + y(img24) * dratdumdd(irmgga)            
     5             + y(in14) * y(ihe4) * dratdumdd(irnag)
...
c..photodisintegration reactions
      dfdy(ife52,iden) = dfdy(ife52,iden) 
     1             + y(ife54) * r1f54dd
     2             - y(ife52) * y(ineut) * y(ineut) * r2f54dd      
     3             + y(ife54) * y(iprot) * y(iprot) * r5f54dd
     4             - y(ife52) * y(ihe4) * r6f54dd
     5             - y(ife52) * y(ihe4) * y(iprot) * r7f54dd  
     6             + y(ini56) * y(iprot) * r8f54dd



c..fe54 reactions
      dfdy(ife54,iden) = -y(ife54) * r1f54dd
     1             + y(ife52) * y(ineut) * y(ineut) * r2f54dd      
     2             - y(ife54) * y(iprot) * y(iprot) * r3f54dd
     3             + y(ini56) * r4f54dd
     4             - y(ife54) * y(iprot) * y(iprot) * r5f54dd
     5             + y(ife52) * y(ihe4) * r6f54dd           
     6             + c54 * y(ini56) * dratdumdd(irn56ec) 
...
c..ni56 reactions
     ...
     4             - y(ini56) * dratdumdd(irn56ec) 



c..photodisintegration reactions
      dfdy(ini56,iden) = dfdy(ini56,iden) 
     1             + y(ife54) * y(iprot) * y(iprot) * r3f54dd
     2             - y(ini56) * r4f54dd 
     3             + y(ife52) * y(ihe4)* y(iprot) * r7f54dd
     4             - y(ini56) * y(iprot) * r8f54dd
...
c..photodisintegration neutrons
      dfdy(ineut,iden) =  2.0d0 * y(ife54) * r1f54dd        
     1             - 2.0d0 * y(ife52) * y(ineut) * y(ineut) * r2f54dd 
     2              + 2.0d0 * y(ihe4) * ralf1dd
     3              - y(ineut)**2 * y(iprot)**2 * ralf2dd
     4              + y(iprot) * dratdumdd(irpen)
     5              - y(ineut) * dratdumdd(irnep)             



c..photodisintegration protons
      dfdy(iprot,iden) = -2.0d0 * y(ife54) * y(iprot)*y(iprot)*r3f54dd
     1             + 2.0d0 * y(ini56) * r4f54dd           
     2             - 2.0d0 * y(ife54) * y(iprot) * y(iprot) * r5f54dd
     3             + 2.0d0 * y(ife52) * y(ihe4) * r6f54dd
     4             + 2.0d0 * y(ihe4) * ralf1dd
     5             - y(ineut)**2 * y(iprot)**2 * ralf2dd
     6             - y(iprot) * dratdumdd(irpen)                
     7             + y(ineut) * dratdumdd(irnep)
...
c..d(itemp)/d(den)
      ...
      subroutine aprox19rat
      include 'implno.dek'
      include 'burn_common.dek'
      include 'network.dek'

c..this routine generates nuclear reaction rates for the aprox19 network.

c..declare  
      integer          i
      double precision rrate,drratedt,drratedd,f1,df1dt,df1dd,
     1                 f2,df2dt,df2dd,sum,dsumdt,dsumdd
...
c..p(n,g)2d 
     ...
     1     ratraw(irhng),dratrawdt(irhng),dratrawdd(irhng),
     2     ratraw(irdgn),dratrawdt(irdgn),dratrawdd(irdgn))

c..2d(p,g)he3
      call rate_dpg(btemp,bden,
     1     ratraw(irdpg),dratrawdt(irdpg),dratrawdd(irdpg),
     2     ratraw(irhegp),dratrawdt(irhegp),dratrawdd(irhegp))

c..he3(n,g)he4  
      call rate_he3ng(btemp,bden,
     1     ratraw(irheng),dratrawdt(irheng),dratrawdd(irheng),
     2     ratraw(irhegn),dratrawdt(irhegn),dratrawdd(irhegn))
...
c..he3(he4,g)be7
     ...
     1     ratraw(ir34),dratrawdt(ir34),dratrawdd(ir34),
     2     rrate,drratedt,drratedd)
...
c..c12(p,g)13n
     ...
     1     ratraw(ircpg),dratrawdt(ircpg),dratrawdd(ircpg),
     2     rrate,drratedt,drratedd)

c..n14(p,g)o15
      call rate_n14pg(btemp,bden,
     1     ratraw(irnpg),dratrawdt(irnpg),dratrawdd(irnpg),
     2     rrate,drratedt,drratedd)

c..fraction fg of n15 goes (pg) to o16, fraction fa of n15 goes (pa) to c12
c..result is  n14+2p  goes to o16    rate= rnpg*fg
c..                   goes to c12+a  rate=rnpg*fa
     ...
     2     rrate,drratedt,drratedd)
      ...
      ratraw(ifa)    = f2/sum
      dratrawdt(ifa) = df2dt/sum - f2/sum**2 * dsumdt
      dratrawdd(ifa) = df2dd/sum - f2/sum**2 * dsumdd
...
c..o16(p,g)f17
     ...
     1     ratraw(iropg),dratrawdt(iropg),dratrawdd(iropg),
     2     rrate,drratedt,drratedd)

c..n14(a,g)f18
      call rate_n14ag(btemp,bden,
     1     ratraw(irnag),dratrawdt(irnag),dratrawdd(irnag),
     2     rrate,drratedt,drratedd)
...
c..fe52(n,g)fe53 
      call rate_fe52ng(btemp,bden,
     1     ratraw(ir52ng),dratrawdt(ir52ng),dratrawdd(ir52ng),
     2     ratraw(ir53gn),dratrawdt(ir53gn),dratrawdd(ir53gn))

c..fe53(n,g)fe54 
      call rate_fe53ng(btemp,bden,
     1     ratraw(ir53ng),dratrawdt(ir53ng),dratrawdd(ir53ng),
     2     ratraw(ir54gn),dratrawdt(ir54gn),dratrawdd(ir54gn))

c..fe54(p,g)co55 
      call rate_fe54pg(btemp,bden,
     1     ratraw(irfepg),dratrawdt(irfepg),dratrawdd(irfepg),
     2     ratraw(ircogp),dratrawdt(ircogp),dratrawdd(ircopg))



c..shut down
c..for fe54 photodisintegration
c      ratraw(ir52ng) = 0.0d0
c      ratraw(ir53gn) = 0.0d0
c      ratraw(ir53ng) = 0.0d0
c      ratraw(ir54gn) = 0.0d0
c      ratraw(irfepg) = 0.0d0
c      ratraw(ircogp) = 0.0d0

c..for he4 photodisintegration
c      ratraw(irheng)  = 0.0d0
c      ratraw(irhegn)  = 0.0d0
c      ratraw(irhng)   = 0.0d0
c      ratraw(irdgn)   = 0.0d0
c      ratraw(irdpg)   = 0.0d0
c      ratraw(irhegp)  = 0.0d0

c..for weak reactions
c      ratraw(irpen)   = 0.0d0
c      ratraw(irnep)   = 0.0d0
c      ratraw(irn56ec) = 0.0d0
...
c..cno cycles
c      ratraw(ircpg)   = 0.0d0
c      ratraw(irnpg)   = 0.0d0
c      ratraw(iropg)   = 0.0d0
c      ratraw(ifa)     = 0.0d0
c      ratraw(ifg)     = 0.0d0
c      ratraw(irnag)   = 0.0d0



c..for a strict alpha chain with only (a,g) and (g,a) reactions
c..shut down the (a,p) (p,a) (p,g) and (g,p) rates 
...
c..will make aprox19 exactly like iso13
...
c..set the log temperature loop limits
       ...
       if (imax .gt. nrattab) stop 'imax too small in aprox19tab'
...
c..photodisintegration n and p back to he4
      dtab(irheng) = bden
      dtab(irhegn) = 1.0d0
      dtab(irhng)  = bden
      dtab(irdgn)  = 1.0d0
...
c..hand finish the three body reactions
      ...
      subroutine weak_aprox19(y)
      include 'implno.dek'
      include 'burn_common.dek'
      include 'vector_eos.dek'
      include 'network.dek'

c..electron capture rates on nucleons for aprox19
c..note they are composition dependent

c..declare
      integer          i
      double precision y(*),zbarxx,ytot1,abar,zbar,ye,xeta,xx
...
c..initialize
      ...
      if (btemp .lt. 1.0e6  .or. bden .lt. 1.0e-9) return



c..with the passed composition, compute abar and zbar
      zbarxx  = 0.0d0
      ytot1   = 0.0d0
      do i=1,ionmax
       ytot1    = ytot1 + y(i)
...
c..get the electron capture rates
      call ecapnuc(xeta,btemp,rpen,rnep,spen,snep)
      ratraw(irpen)   = rpen
      ratraw(irnep)   = rnep


c..ni56 electron capture rate
      call mazurek(btemp,bden,y(ini56),ye,ratdum(irn56ec),xx)
...
c..this routine assumes screen_on = 1 or = 0 and that
c..screen_init = 1 or = 0 has been set at a higer level, 
c..presumably in the top level driver


c..declare
      integer          i,j,k,jscr,init
      double precision y(*),sc1a,sc1adt,sc1add,sc2a,sc2adt,sc2add,
     1                 sc3a,sc3adt,sc3add,abar,zbar,z2bar,ytot1,
     2                 zbarxx,z2barxx
      data             init/1/
...
c..d(p,g)he4
      jscr = jscr + 1
      call screen5(btemp,bden,zbar,abar,z2bar,
     1             1.0d0,2.0d0,zion(ih1),aion(ih1),
     2             jscr,init,sc1a,sc1adt,sc1add)
...
c..pp
      jscr = jscr + 1
      call screen5(btemp,bden,zbar,abar,z2bar,
     1             zion(ih1),aion(ih1),zion(ih1),aion(ih1),
     2             jscr,init,sc1a,sc1adt,sc1add)
...
c..he3 + he4
      jscr = jscr + 1
      call screen5(btemp,bden,zbar,abar,z2bar,
     1             zion(ihe3),aion(ihe3),zion(ihe4),aion(ihe4),
     2             jscr,init,sc1a,sc1adt,sc1add)

      ratdum(ir34)    = ratraw(ir34) * sc1a
      dratdumdt(ir34) = dratrawdt(ir34)*sc1a + ratraw(ir34)*sc1adt
      dratdumdd(ir34) = dratrawdd(ir34)*sc1a + ratraw(ir34)*sc1add

      scfac(ir34)     = sc1a
      dscfacdt(ir34)  = sc1adt
      dscfacdd(ir34)  = sc1add
...
c..c12 + p 
      jscr = jscr + 1
      call screen5(btemp,bden,zbar,abar,z2bar,
     1             zion(ic12),aion(ic12),zion(ih1),aion(ih1),
     2             jscr,init,sc1a,sc1adt,sc1add)
...
c..n14 + p 
      jscr = jscr + 1
      call screen5(btemp,bden,zbar,abar,z2bar,
     1             zion(in14),aion(in14),zion(ih1),aion(ih1),
     2             jscr,init,sc1a,sc1adt,sc1add)
...
c..o16 + p 
      jscr = jscr + 1
      call screen5(btemp,bden,zbar,abar,z2bar,
     1             zion(io16),aion(io16),zion(ih1),aion(ih1),
     2             jscr,init,sc1a,sc1adt,sc1add)
...
c..n14 + alpha 
      jscr = jscr + 1
      call screen5(btemp,bden,zbar,abar,z2bar,
     1             zion(in14),aion(in14),zion(ihe4),aion(ihe4),
     2             jscr,init,sc1a,sc1adt,sc1add)
...
c..declare
      ...
      double precision tt,xin(1),y(ionmax),a1,a2,denom,y1,
     1                 yneut2,yprot2,xx,den1,den2,yy,
     2                 r1,s1,t1,u1,v1,w1,x1,ralf1,ralf2,abar,zbar,
     3                 r1f54,r2f54,r3f54,r4f54,r5f54,r6f54,r7f54,r8f54
...
c..convert the mass fraction to molar fractions
      do i=1,ionmax
       xmass(i) = xin(i)
      enddo
...
c..hydrogen reactions   
      ...
      a1 = y(ihe3) * y(ihe3) * ratdum(ir33) 
      a2 = 0.0d0
      nflowx = nflowx + 1
      string = 'he3(he3,2p)he4'
      call sqeeze(string)
      flonam(nflowx) = string
      flofor(nflowx) = a1
      florev(nflowx) = a2
...
c..(a,p)(p,g) and (g,p)(p,a) reactions
      ...
      a1 = y(ife52) * y(ihe4) * y(iprot) * r7f54
      a2 = y(ihe4) * y(ife52) * r6f54
      nflowx = nflowx + 1
...
c..cno to alfa chain hooks
      a1 = y(in14) * y(ih1) * ratdum(ifa) * ratdum(irnpg)      
      a2 = 0.0d0
      nflowx = nflowx + 1
      string = 'n14(2p,a)c12'
      call sqeeze(string)
      ...
      a1 = y(ihe4) * y(in14) * ratdum(irnag) * 1.5
      a2 = 0.0d0
      nflowx = nflowx + 1
      string = 'n14(1.5a,g)ne20'
      call sqeeze(string)
      flonam(nflowx) = string
      flofor(nflowx) = a1
      florev(nflowx) = a2
...
c..fe52 to fe54
      a1 = y(ini56) * y(iprot) * r8f54
      a2 = y(ife54) * yprot2 * r5f54
      nflowx = nflowx + 1
      string = 'fe52(ag,gp)fe54'
      call sqeeze(string)
      flonam(nflowx) = string
      flofor(nflowx) = a1
      florev(nflowx) = a2
...
c..photodisintegration of helium
      a1 = y(ihe4) * ralf1           
      a2 = yneut2 * yprot2 * ralf2
      nflowx = nflowx + 1
      string = 'a(g)2p2n'
      call sqeeze(string)
      flonam(nflowx) = string
      ...
      subroutine init_aprox19
      include 'implno.dek'
      include 'network.dek'
c..
c..this routine initializes stuff for the aprox19 network
c..
c..declare
      integer          i
