...
************************************************************************
* Computation of solar isotopic abundances                             *
************************************************************************
      ...
      parameter (nam = 240)
      parameter (nnetwz = 17, nnetwa = 35, nsp = 55)
      ...
      common /solar/ fracxs(nsp),xsol,ysol,zsol,zxsol

      open (unit = 10,file = 'abund_gre95')
      open (unit = 20,file = 'solar_abund')
      open (unit = 21,file = 'solar_data')
      open (unit = 22,file = 'solabz.dat')
      open (unit = 23,file = 'solaba.dat')

      do l = 1,nele
         read (10,1000) idum,adum,dum,a(l)
         a(l) = 10.d0**a(l)
      enddo
      ...
      sumzpart = 0.d0
      do l = nnetwz+1,nele
         zmeanz = zmeanz+z(l)*a(l)
         sumzpart = sumzpart+a(l)
      enddo
      zmeanz = zmeanz/sumzpart
      ...
      do i = 1,nele
         do j = 1,ni(i)
            ij = ij+1
            aiso(ij) = a(i)*abiso(ij)
            sumtot = sumtot+aiso(ij)
            relah1(ij) = aiso(ij)/aiso(1)
         enddo
      enddo
      ij = 0
      do i = 1,nele
         ...
         do j = 1,ni(i)
            ij = ij+1
            rela(ij) = aiso(ij)/sumtot
            solabz(i) = solabz(i)+rela(ij)
         enddo
      enddo
      amc1 = 0.5d0
      amc2 = amc1-1.d0
      do m = 1,nam
         ...
         amc2 = amc2+1.d0
         do l = 1,niso
            if (amc1.gt.am(l).and.amc2.lt.am(l))
     &           solaba(m) = solaba(m)+rela(l)
         enddo
      enddo
 ...
 100  sumfracx = 0.d0
      ...
      do i = 1,nele
         do j = 1,ni(i)
            ij = ij+1
            fracx(ij) = rela(ij)*am(ij)*xinvmui
            sumfracx = sumfracx+fracx(ij)
         enddo
      enddo
      delfracx = 1.d0-sumfracx
      if (abs(delfracx).gt.tolfracx) then
         xinvmui = xinvmui/sumfracx
         goto 100
      endif

      xsol = fracx(1)+fracx(2)
      ysol = fracx(3)+fracx(4)
      zsol = 1.d0-xsol-ysol
      zxsol = zsol/xsol

      h2h1 = fracx(2)/fracx(1)
      he3he4 = fracx(3)/fracx(4)
      ...
      do j = nnetwa+1,niso
         ...
         ameanz = ameanz+am(j)*aiso(j)
         sumapart = sumapart+aiso(j)
      enddo
      ameanz = ameanz/sumapart
      ...
      write (20,2000) xsol,ysol,zsol,zxsol,zmeanz,ameanz
      ij = 0
      do i = 1,nele
         do j = 1,ni(i)
            ij = ij+1
            write (20,2100) ij,name(i),int(am(ij)),int(z(i)),abiso(ij),
     &           rela(ij),relah1(ij),fracx(ij)
         enddo
      enddo

      write (21,3000)
      write (21,3100) (fracxs(l),l = 1,5)
      write (21,3100) (fracxs(l),l = 6,10)
      write (21,3100) (fracxs(l),l = 11,15)
      write (21,3100) (fracxs(l),l = 16,20)
      write (21,3100) (fracxs(l),l = 21,25)
      write (21,3100) (fracxs(l),l = 26,30)
      write (21,3100) (fracxs(l),l = 31,35)
      write (21,3100) (fracxs(l),l = 36,40)
      write (21,3100) (fracxs(l),l = 41,45)
      write (21,3100) (fracxs(l),l = 46,50)
      write (21,3200) (fracxs(l),l = 51,nsp)
      write (21,3300)
      write (21,3400) xsol,ysol,zsol
      write (21,3500) h2h1,he3he4

      do i = 1,nele
         write (22,4000) int(z(i)),solabz(i)
      enddo

      do m = 1,nam
         if (solaba(m).gt.1.d-16) write (23,4100) m,solaba(m)
      enddo
 ...
 1000 format (i2,1x,a3,1x,f11.6,1x,f5.2)

 2000 format (/,18x,'SOLAR SYSTEM ISOTOPIC ABUNDANCES',//,
     &     7x,'from: Grevesse, N. & Noels, A.,',/,
     &     13x,'in Origin and Evolution of the Elements,',/,
     &     13x,'eds. N. Prantzos, E. Vangioni-Flam and M. Casse,',/,
     &     13x,'Cambridge University Press (Cambridge), 1993,',
     &     ' p. 14-25',///,
     &     2x,'X = ',0pf8.6,' ; Y = ',0pf8.6,' ; Z = ',0pf8.6,
     &     ' ---> Z/X = ',0pf9.7,/,2x,'Zmean(heavy) = ',0pf7.3,
     &     ' ; Amean(heavy) = ',f9.5,///,
     &     2x,'#',2x,'Name',1x,'A',2x,'Z',3x,'iso. rat.',
     &     2x,'rel. abund.',2x,'abund. H = 1',2x,'mass fraction',/)
 2100 format (1x,i3,2x,a2,1x,i3,1x,i2,2x,0pf9.7,2x,1pe12.6,1x,1pe12.6,
     &     2x,1pe13.7)
 3000 format (/,6x,'data (fracxs(l),l = 1,55) /')
 3100 format (5x,'&',1x,5(1pe11.5,','))
 3200 format (5x,'&',1x,4(1pe11.5,','),1pe11.5,' /')
 3300 format (/,6x,'data xsol,ysol,zsol,h2h1,he3he4 /')
 3400 format (5x,'&',1x,3(1pe13.7,','))
 3500 format (5x,'&',1x,1pe13.7,',',1pe13.7,' /',/)
 4000 format (1x,i2,2x,1pe13.7)
 4100 format (1x,i3,2x,1pe13.7)
