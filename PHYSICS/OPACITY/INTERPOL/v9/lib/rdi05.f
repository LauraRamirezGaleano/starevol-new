	subroutine rdi05(lun,optabe,ntab,nval,nlin,nzva,rlg,
     +                    tlg,opa,ival)
        implicit double precision (a-h,o-z)
        character*(*) optabe
c
c	purpose:
c		read binary OPAL05 tables
c 	reference: 
...
c       
 ...
 1007		 ival(j,k,l) = m
	...
	do l = 1,nzvai-5
c       read line of rlg-values
	   read(lun) (rlg(i,9,l),i=1,nxiri)
c       
c       read 'tlg' + opacity-values
	   do j=1,nyiri
	      read(lun,err=9002)
     &       tlg(j,9,l),(opa(i,j,9,l),i=1,nxiri)
c       
	      do  m=nxiri,1,-1
		 if(opa(m,j,9,l).ne.0.0d0)goto 1008
	      enddo
 1008	      ival(j,9,l) = m
	   enddo
	enddo
...
c       
	      do  m=nxiri,1,-1
		 if(opa(m,j,k,l).ne.0.0d0)goto 1008
	      enddo
 1008	      ival(j,k,l) = m
	   enddo
	   l = l+1
