      ...
      PROGRAM LIVEXTRAPOL_TOT

c     .determine opacity in regions of high logT and logR
c     . new version compatible with the Gz???.x??? opacity tables
      ...
      character*20 name
      character*53 tree
      character*131 dumarray
      character*36 dumarray1

      double precision logT,logR,logrho,kap,ofrac,cfrac

      parameter (nrm=39,nrm0=19,ntm=76,ntm0=70)

      integer ionmax,ishiftR
      parameter (ionmax=21,ntab1=126)

      double precision xsp(ionmax),dx1,dx2
      double precision xmass(ionmax),ymass(ionmax),
     2                 temp,den,abar,zbar,z2bar,ye,
     3                 pin,xin,ein,orad,ocond,opac

      common /tab/ logR(nrm),logT(ntm),cof(ntm,nrm)
      common / azion / aion(ionmax),zion(ionmax) 

      dimension xfrac(ntab1),yfrac(ntab1),zfrac(ntab1),cfrac(ntab1),
     &     ofrac(ntab1)

      read (*,*) name

      tree = '/Users/td23/STAREVOL/EVOL/PHYSICS/OPACITY/LIB_AY18/'

c      open (unit = 10,file = trim(tree) // trim(name) )
      open (unit = 10,file =
     $     '/Users/td23/STAREVOL/EVOL/PHYSICS/OPACITY/LIB_AY18/azer')

      open (unit = 20,file = 'Az'//trim(name) )

      ishiftR = 2

c     read chemical composition
      do i = 1,36
         read (10,'(a131)') dumarray
      enddo
      do i = 3,5
         read (10,92) xsp(i)
         ymass(i) = xsp(i)/aion(i)
      enddo
      read (10,'(a131)') dumarray
      do i = 6,16
         read (10,92) xsp(i)
      ...
      read (10,'(a131)') dumarray
      read (10,92) xsp(21)
      ymass(21) = xsp(21)/aion(21)
 92   format(59x,0pf8.6,7x,0pf8.5)
      read (10,'(a131)') dumarray
      read (10,'(28x,i3)') ntab
      read (10,'(a131)') dumarray


c     read tables summaries
      do i = 1,ntab
         read (10,1000) xfrac(i),yfrac(i),zfrac(i),cfrac(i),ofrac(i)
      enddo
 1000 format (33x,f6.4,3x,f6.4,3x,f6.4,5x,f6.4,5x,f6.4)

      rewind (10)

      do i = 1,240
         read (10,'(a131)') dumarray
         write (20,'(a)') dumarray(1:lenc(dumarray))
      enddo
      ...
      logT(ntm0+4) = 9.5d0
      logT(ntm0+5) = 9.7d0
      if (ntm0+6.ne.ntm) stop ' ntm bad dimension'
      logT(ntm) = 9.9d0
...
c     initialize new table
         ...
         read (10,'(f10.5)') dum
         write (20,'(1x)')
         read (10,111) dumarray1,x,y,z,xc,xo
 111     format (a36,3(0pf6.4,3x),2x,0pf6.4,5x,0pf6.4)
         write (20,112) dumarray1,x,y,z,xc,xo
 112     format(a36,f6.4,' Y=',f6.4,' Z=',f6.4,' dX1=',f6.4,
     &        ' dX2=',f6.4)
         read (10,'(f10.5)') dum
         write (20,'(1x)')
         read (10,'(a131)') dumarray
         ...
         read (10,'(f10.5)') dum
         write (20,'(1x)')
         read (10,'(4x,f6.1,18f7.1)') (logR(kk),kk = 1,nrm0)
         logR(nrm0+1) = 1.5d0
         logR(nrm0+2) = 2.0d0
         logR(nrm0+3) = 2.5d0
         logR(nrm0+4) = 3.0d0
         ...
         logR(nrm0+17) = 9.5d0
         logR(nrm0+18) = 10.d0
         do k = nrm,ishiftR+1,-1
            logR(k) = logR(k-ishiftR)
         enddo
         logR(ishiftR) = -8.5d0
         logR(ishiftR-1) = -9.d0
         write (20,'(a4,f6.1,38f7.1)') 'logT',(logR(kk),kk = 1,nrm)
c         write (20,'(a4,f6.1,27f7.1)') 'logT',(logR(kk),kk = 1,nrm)
c        write (20,'(a4,f6.1,25f7.1)') 'logT',(logR(kk),kk = 1,nrm)
         read (10,'(f10.5)') dum
         write (20,'(1x)')

         do k = 1,ntm0
            read (10,'(f4.2,19f7.3)') logT(k),(cof(k,l+ishiftR), 
     &           l = 1,nrm0)
         enddo


c     .determine table chemical composition
         xmass(1) = xfrac(i)
         xmass(2) = yfrac(i)
         xsolc = zfrac(i)*xsp(3)+1.d-30
         xsolo = zfrac(i)*xsp(5)+1.d-30
         xmass(3) = xsolc+cfrac(i)
         xmass(4) = zfrac(i)*xsp(4)+1.d-30
         xmass(5) = xsolo+ofrac(i)

         do k = 6,ionmax
            xmass(k) = zfrac(i)*xsp(k)+1.d-30
         enddo

         call azbar(xmass,aion,zion,ionmax,ymass,abar,zbar)

c..   determine opacity in regions of large values of logR
         do k = 1,ntm
            do l = 1,nrm
               temp_row(1) = 10.d0**logT(k)
               logrho = logR(l)-18.d0+3.d0*logT(k)
               den_row(1) = 10.d0**logrho 
               abar_row(1) = abar
               zbar_row(1) = zbar
               
c..   here the eos pipeline is only 1 element long
               jlo_eos = 1
               jhi_eos = 1

c..   call an equation of state
c..   you can download this eos from fxt's eos code webpage
c..   see the end of this file for how its brought in

               call eosfxt

c..   one *must* always call the eos before calling the opacity routine 
c..   since some of the required input must come from an eos


c..   load the electron-positron pressure, electron-positron number densities,
c..   and electron chemical potential from the eos
               pin   = pele_row(1) + ppos_row(1)
               xin   = xne_row(1) + xnp_row(1)
               ein   = etaele_row(1)

c..   call the opacity routine
               call sig99(temp_row(1),den_row(1),xmass,zion,aion,
     &              ionmax,pin,xin,ein,orad,ocond,opac)
               if (cof(k,l).eq.9.999d0.or.l.le.
     &              ishiftR.or.l.gt.(nrm0+ishiftR).or.k.gt.ntm0) then
                  if (orad.lt.0.d0) then
                     opacity = ocond
                  else
                     opacity = opac
                  endif
               else
                  opacity = 1.d0/(1.d0/10.d0**cof(k,l)+1.d0/ocond)
               endif
               if (opacity.lt.0.d0.or.opacity.gt.1.d10) then
                  if (opac.lt.0.d0.and.ocond.gt.0.d0) then
                     cof(k,l) = log10(ocond)
                  else
                     cof(k,l) =  9.9989d0
                     write(*,*) 'WARNING',temp_row(1),den_row(1),pin
     &                    ,xin,ein,orad,ocond,opac,opacity,k,l
                  endif
               else
                  cof(k,l) = max(log10(opacity),-9.9989d0)
               endif
            enddo
         enddo
         do k = 1,ntm
            write (20,'(f4.2,39f7.3)') logT(k),(cof(k,l), l = 1,nrm)
c            write (20,'(f4.2,28f7.3)') logT(k),(cof(k,l), l = 1,nrm)
         enddo
      enddo
...
*     Lists the A and Z for each of the nucleides contributing to 
*     the opacity. 
*     Used for the new opacity tables Gz??.x?? where A is not given
*     anymore. The values entered are from the old tables Ox??.z??
*
*     Version: May 2003
*
**********************************************************************
      ...
      integer i,ionmax!,ntab1
      
      parameter (ionmax=21)

      double precision aion,zion

      common / azion / aion(ionmax),zion(ionmax) 

      data (aion(i),i = 1,ionmax) / 1.0079d0,4.0026d0,12.0110d0,
     &     14.0067d0,15.9994d0,20.1790d0,22.98977d0,24.3050d0,
     &     26.98154d0,28.0855d0,30.97376d0,32.0600d0,35.4530d0,
     &     39.9480d0,39.0983d0,40.0800d0,47.9000d0,51.9960d0,54.9380d0,
     &     55.8470d0,58.70000d0 /

      data (zion(i), i = 1,ionmax) / 1.d0,2.d0,6.d0,7.d0,8.d0,10.d0,
     &     11.d0,12.d0,13.d0,14.d0,15.d0,16.d0,17.d0,18.d0,19.d0,20.d0,
     &     22.d0,24.d0,25.d0,26.d0,28.d0 /
...
************************************************************************
      ...
      save
c.. 
c..this routine approximates the thermal transport coefficients.
c..
c..input:
c..temp   = temperature temp (in K)
c..den    = density den (in g/cm**3)
c..ionmax = number of isotopes in the composition
c..xmass  = mass fractions of the composition
c..zion   = number of protons in each isotope (charge of each isotope)
c..aion   = number of protons + neutrons in each isotope (atomic weight)
c..pep    = electron-positron pressure (in erg/cm**3)
c..xne    = electron-positron number density (in 1/cm**3)
c..eta    = electron degeneracy parameter (chemical potential / k T)
c..
c..output:
c..orad   = radiation contribution to the opacity (in cm**2/g)
c..ocond  = conductive contribution to the opacity (in cm**2/g)
c..opac   = the total opacity (in cm**2/g)
c..
c..declare the pass
      integer          ionmax
...
c..declare the internal variables
     ...
     1                 xhe,xz,xkc,xkap,xkb,xka,dbar,oiben1,d0log,xka1,
     2                 xkw,xkaz,dbar1log,dbar2log,oiben2,t4,t4r,t44,t45,
     3                 t46,ck1,ck3,ck2,ck4,ck5,ck6,xkcx,xkcy,xkcz,ochrs,
     4                 th,fact,facetax,faceta,ocompt,tcut,cutfac,xkf,
     5                 dlog10,zdel,zdell10,eta0,eta02,thpl,thpla,cfac1,
     6                 cfac2,oh,pefac,pefacl,pefacal,dnefac,wpar2,walf,
     7                 walf10,thx,thy,thc,farg,ffac,xmas,ymas,wfac,cint,
     8                 vie,cie,tpe,yg,xrel,beta2,jy,vee,cee,ov1,ov,drel,
     9                 drel10,drelim,t6_switch1,t6_switch2,x,x1,x2,
     &                 alfa,beta



c..various physical and derived constants
c..con2 = con1*sqrt(4*pi*e*e/me)  
c..meff = hbar/(me*c)*(3*pi**2)**(1/3) 
c..weid = (pi*kerg)**2/(3*me)     
c..iec  = 4*e**4*me/(3*pi*hbar**3) 
c..xec  = hbar/kerg*e*sqrt(4*pi/me) 

      double precision third,twoth,pi,avo,c,ssol,asol,zbound,t7peek,
     1                 con2,k2c,meff,weid,iec,xec,rt3
      parameter (third = 1.0d0/3.0d0, 
     1          twoth  = 2.0d0 * third, 
     2          pi     = 3.1415926535897932384d0, 
     3          avo    = 6.0221367d23, 
     4          c      = 2.99792458d10, 
     5          ssol   = 5.67050407222d-5, 
     6          asol   = 4.0d0*ssol/c, 
     7          zbound = 0.1d0, 
     8          t7peek = 1.0d20, 
     9          k2c    = 4.0d0/3.0d0*asol*c, 
     &          meff   = 1.194648642401440d-10, 
     1          weid   = 6.884326138694269d-5, 
     2          iec    = 1.754582332329132d16,  
     3          xec    = 4.309054377592449d-7,
     4          rt3    = 1.7320508075688772d0, 
     5          con2   = 1.07726359439811217d-7)


c..switches for the iben & christy regimes
       parameter        (t6_switch1 = 0.5d0, t6_switch2 = 0.9d0)
c       parameter        (t6_switch1 = 1.0d0, t6_switch2 = 1.5d0)
...
c..from iben apj 196 525 1975
     ...
     1     .not.((xh.lt.1.0e-5) .and. (xz.gt.zbound)) ) then 
     ...
     1        * (log10(t6) - 0.22d0 + 0.1375d0*xh)**2)  
       xkw  = 4.05d0 * exp(-(0.306d0  - 0.04125d0*xh)
     1        * (log10(t6) - 0.18d0 + 0.1625d0*xh)**2) 
...
c..from christy apj 144 108 1966
...
c..opacity in presence of hydrogen  
...
c..opacity in absence of hydrogen   
      else  
...
c..add in the compton scattering opacity, weaver et al. apj 1978 225 1021
...
c..cutoff radiative opacity when 4kt/hbar is less than the plasma frequency  
      tcut = con2 * sqrt(xne) 
...
c..fudge molecular opacity for low temps
      xkf    = t7peek * den * (temp * 1.0d-7)**4
      orad   = xkf * orad/(xkf + orad)


c..conductivity section:
c..drel is the dividing line between nondegenerate and degenerate regions,
c..taken from clayton eq. 2-34. if the density is larger than drel, then
c..use the degenerate expressions. if the density is smaller than
c..drelim, use the non-degenerate formulas. in between drel and drelim,
c..apply a smooth blending of the two.
...
c..from iben apj 196 525 1975 for non-degenerate regimes
...
c..pefac and walf factors
     ...
     1          (den*avo*(w(4)+w(5)+w(6))/xne + dnefac)/(sqrt(t6)*pefac)
       walf   = 0.5d0 * log(wpar2)    
       walf10 = 0.5d0 * log10(wpar2)    

c..thx, thy and thc factors
...
c..from yakovlev & urpin soviet astro 1980 24 303 and 
c..potekhin et al. 1997 aa 323 415 for degenerate regimes
...
c..ion-electron collision frequency and the thermal conductivity 
       vie   = iec * zbar * ymas * cint
       cie   = wfac/vie 

c..electron-electron collision frequency and thermal conductivity 
     ...
     1        * ( yg**3 / (3.0d0 * (1.0d0 + 0.07414 * yg)**3) 
     2        * log((2.81d0 - 0.810*beta2 + yg)/yg)    
     3            + pi**5/6.0d0 * (yg/(13.91d0 + yg))**4 )
       vee = 0.511d0 * temp**2 * xmas/ymas**2 * sqrt(xmas/ymas) * jy 
       cee = wfac/vee

c..total electron thermal conductivity and conversion to an opacity 
       ov1   = cie * cee/(cee + cie)
       ov    = k2c/(ov1*den) * temp**3 
      end if 

c..blend the opacities in the intermediate region
...
c..for the uniform background coulomb correction
     ...
     4                 aion,
     5                 lami,inv_lami,lamidd,lamida,lamidz,
...
************************************************************************
      ...
      SUBROUTINE KAPPA_FITS (X,Y,XC,XO,Z,kap,logT,logR)

************************************************************************

      implicit double precision (a-z)
      
      logrho = logR-18.d0+3.d0*logT
      rho = 10.d0**(logrho)
      T6 = 10.d0**logT*1.d-6

c     mixture :
c     X=0,Y=0.5, XC=0.5
c     X=0,Y=0,XC=0.5,XO=0.5
c     X=0,Y=0,XC=0,XO=1
c     X=0,Y=0.9999,Z=0.0001  <-- not needed : to be checked

      mu = X+Y+XC*3.d0+XO*4.d0+Z*18.d0-1.d0
c      write(*,*) mu,X,Y,XC,XO,Z
      a = (1.25d0+0.488d0*sqrt(mu)+0.092d0*mu)/0.67d0
      b = 3.86d0+0.252d0*sqrt(mu)+0.018d0*mu
      logrhop=-a+b*log10(T6)
      c = (2.019d-4*rho/t6**1.7)**2.425d0
      ap = 1.d0+c*(1.d0+c/24/55d0)
      logkp=0.67d0*(logrho-logrhop)+log10(ap)

      d = 0.05d0*(log10(T6)-1.7d0)
      ke = (0.2d0-d-sqrt(d*d+0.0004d0))*(1+X)

      kap = log10(ke+10.d0**logkp)

c      write(*,*) X,Z,Y,XC,XO,ke,kap,logT,logR
      return

c     mixture:
c     X=0.8,Y=0.18,Z=0.02
c     X=0,Y=0.98,Z=0.02
c     X=0.8,Y=0.1999,Z=0.0001
c     X=0,Y=0.9999,Z=0.0001

      r1 = -2.3d0+4.8333d0*log10(T6)
      r2 = -(1.86d0+0.3125d0*X)+3.86d0*log10(T6)
      logrhop = min(r1,r2)
      a1 = 1.22d0*exp(-(1.74-0.755*X)*(log10(T6)-0.22d0+0.1375*X)**2)
      w = 4.05d0*exp(-(0.306d0-0.004125*X)*(log10(T6)-0.18d0+
     &     0.1625d0)**2)
      if (log10(T6).ge.0.d0) then
         logrho0 = -(1.68d0+0.35d0*X)+1.8d0*log10(T6)
      else
         logrho0 = -(1.68d0+0.35d0*X)+(3.42d0-0.52d0*log10(T6))         
      endif
      az0 = a1*exp(-2.76d0*(logrho-logrho0)**2/W**2)
      az = az0*(Z/0.02d0)
      logkp = 0.67d0*(logrho-logrhop)+az
