...
c     . new version compatible with the Gz???.x??? opacity tables
     ...
     3                 pin,xin,ein,orad,ocond,opac
      ...
      integer ir,itemp,nrm,ntm,k,itable,nchem,io16,ine,isi,ichem,
     &     ispec1,ispec2,ispec3,ntable
      double precision rogrid,tgrid,xgrid

      parameter (nchem=11,ntm = 11,nrm=17)

      dimension xgrid(nchem),tgrid(ntm),logR(nrm),logT(ntm)
      dimension cof(ntm,nrm)
...
c..iron 56
      ...
      do k = 1,ionmax
         ymass(k) = xmass(k)/aion(k)
      enddo

c..   define temperature grid : 1e9--> 5e9
      do k = 1,ntm
         logT(k) = dble(k-1)*0.1d0+8.5d0
         tgrid(k) = 10.d0**logT(k)
      enddo
c..   define density grid : log(ro) : 1e4 --> 1e10
      do k = 1,nrm
         logR(k) = -7.d0+dble(k-1)*0.5d0
      enddo
c..   define elements mass fraction
      do k = 1,nchem
         xgrid(k) = dble(k-1)*.1d0
      enddo

***   loop on table : itable

      open(unit=32,file='hthopa.f')
      write (32,1000)
      write (32,1100)
      write (32,1200) (logR(l),l=1,nrm)
      write (32,1300) (logT(l),l=1,ntm)
 1000 format (/,'      BLOCK DATA HTHOPA',/)
 1100 format(6x,'implicit none',//,6x,'double precision rkh,tkh,',
     &     'opachth',//,6x,'integer km,kt',//,6x,'common /hthopac/ ',
     &     'rkh(17),tkh(11),opachth(17,11,198)',/)
 1200 format(6x,'data (rkh(km),km = 1,17)/',6(f5.2,','),/,5x,'&',1x,
     &     10(f5.2,','),/,5x,'&',1x,f5.2,'/')
 1300 format(6x,'data (tkh(kt),kt = 1,11)/',6(f5.2,','),/,5x,'&',1x,
     &     4(f4.2,','),f5.2,'/')
      ntable=33
      idx = 0
      do itable =1,ntable
...
c..   table O-Si-Fe
         if (itable.ge.12.and.itable.le.22) then
            as1 = ' O'
            as2 = 'Ne'
            as3 = 'Si'
            ispec1 = io16
         ...
         if (itable.gt.11) itab = itable-11
         if (itab.gt.11) itab = itab-11
         if (itable.eq.1.or.itable.eq.12.or.itable.eq.23) then
            write (32,1500) as1,as2
 1500       format (6x,/,71('*'),/,'*',6x,'TABLE enriched in ',a2,'-',a2
     &           ,/,71('*'))
         endif

***   inside a table loop on the chemical composition
         nxs = 12-itable
         if (itable.ge.23) nxs = nxs+11
         if (nxs.lt.1) nxs = nxs+11
         if (itable.ge.23) then
            xmass(ispec3) = xgrid(nxs)
            ymass(ispec3) = xmass(ispec3)/aion(ispec3)
            write (32,12) as3,xgrid(nxs)
 12         format (/,72('*'),/,'*',5x,'[',a2,'] ',0pf5.3,/,72('*'))
            write (*,11) as3,xgrid(nxs)
 11         format('  [',a2,'] : ',0pf5.3)
         endif
         if (itable.lt.23.and.itable.ge.12) then
            xmass(ispec3) = xgrid(nxs)
            ymass(ispec3) = xmass(ispec3)/26.d0
            write (32,312) xgrid(nxs)
 312        format (/,72('*'),/,'*',5x,'[Mg-Si] ',0pf5.3,/,72('*'))
            write (*,311) xgrid(nxs)
 311        format('  [Mg-Si] : ',0pf5.3)
         endif
          if (itable.lt.12) then
            xmass(ine) = xgrid(nxs)
            ymass(ine) = xmass(ine)/aion(ine)
            write (32,412) xgrid(nxs)
 412        format (/,72('*'),/,'*',5x,'[Ne] ',0pf5.3,/,72('*'))
            write (*,411) xgrid(nxs)
 411        format('  [Ne] : ',0pf5.3)
         endif
        do ichem = 1,itab
            xmass(ispec1) = xgrid(ichem)
            ymass(ispec1) = xmass(ispec1)/aion(ispec1)
            xmass(ispec2) = 1.d0-xmass(ispec3)-xmass(ispec1)
            ymass(ispec2) = xmass(ispec2)/aion(ispec2)
            write (*,1111) as3,xmass(ispec3),xmass(ispec1),as2,
     &           xmass(ispec2)
            call azbar(xmass,aion,zion,ionmax,ymass,abar,zbar)
 1111       format(a2,0pf7.3,', O ',0pf6.3,', ',a2,' ',0pf6.3)
            abar_row(1) = abar
            zbar_row(1) = zbar

***   loop on the temperature
            write (32,111) as1,xmass(ispec1),as2,xmass(ispec2)
 111        format (/'*',6x,'Table : ',a2,' = ',f4.2,' and ',a2,
     &           ' = ',f4.2,/)
...
c..set the input vector
               temp_row(1) = tgrid(itemp)

               write (32,112) log10(tgrid(itemp))
 112           format ('*',6x,'log T = ',f5.2)

***   loop on the density
               do ir = 1,nrm

                  den_row(1)  = logR(ir)-18.d0+3.d0*log10(tgrid(itemp))
                  den_row(1) = 10.d0**den_row(1)
...
c..   call the opacity routine
     ...
     &                 ionmax,pin,xin,ein,orad,ocond,opac)
                  if (orad.lt.0.d0) then
                     cof(itemp,ir) = max(log10(ocond),-9.9989d0)
                  else
                     cof(itemp,ir) = max(log10(opac),-9.9989d0)
                  endif
               enddo
               write (32,113) itemp,idx,(cof(itemp,l),l=1,nrm)
            enddo
         enddo
      enddo
 113  format (6x,'data (opachth(km,',i2,',',i3,'),km = 1,17)/',
     &     4(f6.3,','),/,5x,'&',1x,9(f6.3,','),/,5x,'&',1x,3(f6.3,','),
     &     f6.3,'/') 
