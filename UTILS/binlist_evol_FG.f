...
C      double precision dtcour,lmax,tmax,enucmax
      ...
      double precision tc_mat, rc_mat
...
!     &        ,0.3d0]
     ...
     &        ,0.3d0,0.5d0]
         do ij = 1,nb_Zeff
            ...
            if (ij.eq.nb_Zeff.or.(ij.eq.(nb_Zeff-1))) nameatm(17:17)='+'
            nameatm(18:20)=charZ
            nameZ = nameatm
            do j = 1,nb_geff
               nameatm = nameZ
               ...
               nameatm(11:13)=charg
               do i = 1,nb_Teff
                  if ((Ttabeff(i).eq.10000.and.gtabeff(j).eq.2.0
     &                 .and.Ztabeff(ij).eq.0.0).or.
     &                 (Ttabeff(i).eq.10000.and.gtabeff(j).eq.2.0
     &                 .and.Ztabeff(ij).eq.-2.0).or.
     &                 (Ttabeff(i).eq.10000.and.gtabeff(j).eq.4.0
     &                 .and.Ztabeff(ij).eq.-2.0).or.
     &                 (Ttabeff(i).eq.3000.and.gtabeff(j).eq.3.5
     &                 .and.Ztabeff(ij).eq.-3.5).or.
     &                 (Ttabeff(i).eq.4000.and.gtabeff(j).eq.4.5
     &                 .and.Ztabeff(ij).eq.-3.5).or.
     &                 (Ttabeff(i).eq.3900.and.gtabeff(j).eq.4.5
     &                 .and.Ztabeff(ij).eq.-3.5).or.
     &                 (Ttabeff(i).eq.10000.and.gtabeff(j).eq.2.0
     &                 .and.Ztabeff(ij).eq.-4.0).or.
     &                 (Ttabeff(i).eq.10000.and.gtabeff(j).eq.4.0
     &                 .and.Ztabeff(ij).eq.-4.0))  then 
                     rtau(:,j,i,ij) = 0.d0
                     rT(:,j,i,ij) = 0.d0
                     rhotab(:,j,i,ij) = 0.d0
...
c.. Smoothing the soundspeed profile
 ...
 45   format (/,' Total number of shells :',i5,', model number :',i8,/
     &     ' Age = ',1pe12.6,' yr',/)
      ...
      inquire(file="/home/spectro/gallet/STAREVOL/PROFIL/"//
     &     "profil.dat", exist=exist)
      if (exist) then
       	open(unit=999,file="/home/spectro/gallet/STAREVOL/PROFIL/"//
     &     "profil.dat",form='formatted',status="old")
      else
    	open(unit=999,file="/home/spectro/gallet/STAREVOL/PROFIL/"//
     &     "profil.dat",form='formatted',STATUS='new')
      end if
     
         
         write(999,*) "********"
         write(999,*) "nphase,time =",nphase, time*seci
         write(999,*) "Rc(r)/R_*, tc Omega Rc_top l_c v_c " 
...
*------------------------------------------------------
      ...
      if (xsp(1,ih1).gt.1.d-5) then 
          call diffpaq(ihe4,nmod,1,vdmic,dmic,xsp,ratneh)
      endif
...
c integreted turmover time (bottom and top layers are not taken into account)
 ...
 6666    format('CE-TURNOVER ',i8,1x,1pe16.10,11(1x,1pe10.4))
         ...
         ! FAIRE CALCUL QUANTITE MATHIS ICI 
         ! CALCUL DE ECKMAN ET ROSSBY
         !FAIRE BOUCLE SUR RADIUS
         !FAIRE UN TEST SUR AGE AVANT => chercheur début, milleux et fin PMS, milieux et fin MS approximativement!
         ! En gros prendre age sur PMS et age sur MS et diviser par deux, pour chaque étoile.
         ...
         k = novlim(1,3)
         do while (r(k).lt.r(novlim(1,4)) .and.
     &           k.lt.novlim(1,4))
         	...
         	if (sconv(k).eq.0.d0) then
         		...
         		tc_mat = alphac*hp(k)/sconv(k)
         		rc_mat = r(k) 
         	endif	
            
            
         	secd = 3600.d0*24.d0
         	write(999,*) r(k)/r(nmod), tc_mat/secd,
     &       	vomega(k),r(novlim(1,4)), alphac*hp(k), sconv(k)
...
C Modif CC ondes (11/07/07)
         ii = nmod-i+1
         write (13,3210) i,tconv(i),abs(rsconv(i)),fconv(i)/flux,
     &        frad(i)/flux,renucl(i),enupla(i),egrav(i),enunucl(i),
C Modif CC ondes (12/04/07) -->
     &        Dconv(i),dturb(i),dherw(i),Dsc(i),dmic(i),vdmic(i),
C Modif CC ondes (11/07/07)
C     &        Dthc(i),Dondes(i),Dondeschim(i),iscr
     &        Dthc(i),Dondes(i),Dondes(i),lumwave(ii),iscr
     &        
      enddo
...
c.. *.p10
 ...
 3460 format (1x,i4,1x,1pe12.5,1x,1pe12.5,4(1x,1pe10.4),3(1x,
     &     1pe11.4),3(1x,1pe10.4),12(1x,1pe11.4),1x,(1pe12.5))
