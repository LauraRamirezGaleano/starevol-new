...
C      double precision dtcour,lmax,tmax,enucmax
      ...
      double precision Ri_n,S_n,Y_n,lumwaves(nsh),xpsi_out(nsh) !Added LR LR 20250616 
...
c      double precision sigc
      ...
      ! integer :: ios !LR DIAGNOSTIC
      ! character(len=256) :: file_2c !LR DIAGNOSTIC
...
c.. Smoothing the soundspeed profile
   ...
   !          enddo
     ...
     &              Dondes(k), Dondeschim(k), lumwaves(k), 
     &              lumwave_core(k), lumondes_surf(k), lumondes_core(k), 
     &              lumondestot(k), xintret_target(k), vr(k), vxpsi(k), !brunt(k), ! LR 20250617
     &              Dmicro(k), vmicro(k), Dthc(k), phiKS(k), 
     &              deltaKS(k), tautime(k,ippg), Ereac(k,ippg), 
     &              tautime(k,ipdg), Ereac(k,ipdg), tautime(k,i2he3), 
     &              Ereac(k,i2he3), tautime(k,ihe3ag), Ereac(k,ihe3ag), 
...
c      call lissage (dife,1,nmod-1,5)
     ...
     &        name(1:lenci(name)) //cmodel//'.p10',status = 'unknown')
         write (20,1220)
         do i = 1,nmod
            ! if (abs(xpsis(i)) < 1.0d-30) then
            !    xpsi_out = 0.0d0
            ! else
            !    xpsi_out = xpsis(i)
            ! endif
            write (20,3460) i,vomega(i),urs(i)*ur_Ss,dift(i),dife(i),
     &           Dtot(i),Dhold(i),xpsi_out(i),xlambdas(i),abmuj(i),
...
c.. fichier onde .p14
   ...
   !   &           deltaKS(k)
     ...
     &           lumwaves(k),lumwave_core(k),
     &           lumondes_surf(k),lumondes_core(k),lumondestot(k),
     &           xintret_target(k),vr(k),vxpsi(k), !brunt_o(k),
     &           Dmicro(k),vmicro(k),Dthc(k),phiKS(k),
     &           deltaKS(k)
         enddo
         close (26)
...
c..   old version < 2.10
 ...
 2626 format('# nsh',1x,'lgdmic',6x,'lgdturb',5x,'vom',9x,'depottot',1x
     ...
     &     ,'Dondes',2x,'Dondeschim',2x,'lumwaves',2x,'lumwave_core',1x
     &     ,'lumondes_surf',1x,'lumondes_core',1x,'lumondestot',1x
     &     ,'tau',5x,'vr',10x,'vxpsi',7x,'Dmicro2',5x,'vmicro2',3x !brunt_o
