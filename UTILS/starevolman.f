...
*                                                                      *
* $LastChangedDate:: 2014-02-04 21:59:25 +0100 (Tue, 04 Feb 2014)    $ *
* $Author:: amard                                                    $ *
* $Rev:: 23                                                          $ *
*                                                                      *
************************************************************************
      ...
      integer izchange,iheavy!,refsolchem ! maj TD
      integer idiffvr
     ...
     &     FeHsol,fracFe,iheavy
      ...
      print *, 'refsolar starevolman.f', refsolar
      ! Maj TD 07/2021
      write (*,*) 'Initial choice for solar reference and input physisc'
 110  format (/,2x,'Choose set of reference chemical solar',
     &        ' abundances')
 500     write (*,501)
 501     format (/,5x,'Choose your option:',//
     &        3x,'o GN93, no micro, no rot, grey atm       [01]',/,
     &        3x,'o AGS05, no micro, no rot, grey atm      [02]',/,
     &        3x,'o AGSS09, no micro, no rot, grey atm     [03]',/,
     &        3x,'o Grid, no micro, no rot, grey atm       [04]',/,
     &        3x,'o AGSS09, no micro, no rot, PHOENIX atm  [05]',/,
     &        3x,'o AGSS09, no micro, no rot, KS66 atm     [06]',/,
     &        3x,'o AGSS09, micro, no rot, grey atm        [07]',/,
     &        3x,'o AGSS09, micro, no rot, grey KS66       [08]',/,
     &        3x,'o AGSS09, micro, rot, grey atm           [09]',/,
     &        3x,'o AGSS09, micro, rot, PHOENIX atm        [10]',/,
     &        3x,'o AGSS09, micro, rot, KS66 atm           [11]',/,
     &        3x,'o AY18, no micro, no rot, KS66 atm       [12]',/,
     &        3x,'o AY18, micro, no rot, KS66 atm          [13]',/,
     &        3x,'o AY18, micro, rot, KS66 atm             [14]',//,
     &        '    ---> Enter option',5x,i2)
c         read (5,'(i2)') refsolar
c         print *,trim(refsolar)
         read (5,'(i2)') refsolchem
         print *,refsolchem
      ...
      ! AGS05   
         ...
         refsolar = 'AGS05'
      ! AGSS09   
      else if (refsolchem.eq.3.or.refsolchem.ge.5.and.refsolchem.le.11)
     $        then
         if (refsolchem.eq.9) then
         ...
         refsolar = 'AGSS09'
      ! Grid   
      else if (refsolchem.eq.4) then
         xFesol = 1.147417671020920d-3
         print *, 'Grid'
         refsolar = 'Grid'
      ! AY18   
      else if (refsolchem.ge.12.and.refsolchem.le.14) then
         xFesol = 1.1947208819D-03
         print *, 'AY18'
         refsolar = 'AY18'
      else
...
c     Coc & Vangioni (2017, JMPE, 2641002C) !modif TD 2018
      xini = 0.7516d0
      yini = 0.2484d0
      zini = 0.d0
...
c      iret=system("clear" )
 88   write (*,100) 
 100  format(/,5x,18('*'),/,5x,'MODEL FILE MANAGER',/,5x,18('*'),//,
     &     'default input : modini.bin',/,
     &     /,'choose your option :',/
     &   2x,'o global  METALLICITY change           [1]',/,
     &   2x,'o change  STELLAR MASS  (homology)     [2]',/,
     &   2x,'o initial ROTATION files               [3]',//,
     &   2x,'o convert Z <-> [Fe/H]                 [4]',/,
     &   2x,'o determine MAIN SEQUENCE DURATION     [5]',//,
     &   2x,'o binary --> binary modification       [6]',/,
     &   2x,'o extract shell information            [7]',/,
     &   2x,'o binary --> ascii conversion          [8]',//,
     &   2x,'o change abundance (C, O, Na, He)      [9]',//,
     &   2x,'o convert psi <-> theta                [a]',/,
     &   2x,'o convert solar abund to Xsol          [b]',/,
     &   2x,'o check mass distribution              [c]',/,
     &   2x,'o update solar comp to Asplund-Cunha   [d]',//,
     &   2x,'o exit                                 [x]',//,
     &   '  ---> enter option',5x,a1)
      read (5,'(a1)') opt
      if (opt.eq.'x'.or.opt.eq.'q') stop '       end starevolman'
      if (opt.ne.'1'.and.opt.ne.'2'.and.opt.ne.'3'.and.opt.ne.'4'
     &     .and.opt.ne.'5'.and.opt.ne.'6'.and.opt.ne.'7'.and.
     &     opt.ne.'8'.and.opt.ne.'9'.and.opt.ne.'a'.and.opt.ne.'b'
     &     .and.opt.ne.'c'.and.opt.ne.'d') goto 88

      if (opt.eq.'1'.or.opt.eq.'9') then
c         iret=system("clear" )
         write (*,*)
         write (*,*) 'METALLICITY CHANGE'
         write (*,180) 
 180     format (/,2x
     $        ,'Do you want to use the solar chemical composition',
     &        ' (y or n)?')
         read (5,*) rep
c..   If use the chemical mix from the solar calibration for
c..   the selected solar composition, use directly values from evodat to
c..   initialize the mass fractions xsp.
c..   This procedure is not valid if SOLCOMP = GRID
         
         solar = (rep.eq.'n'.or.(rep.eq.'y'.and.refsolchem.eq.4))  ! si non solaire, solar = .true.
         if (solar) then 
            write (*,200) 
            open (unit = 20, err=222, file = 'modini.bin',
     &           form = 'unformatted', status = 'old')
 200        format (/,2x,'Do you want to change Z (1) or [Fe/H] (2)')
            read (5,*) izchange
            if (izchange.eq.1) then 
               write(*,210)
 210           format (/,2x,'enter new Z ( < 0 for solar composition)')
               read (5,*) znew
               if (znew.lt.0) then
                  write (*,401)
                  read (5,'(a6)') refsolar
c                  call solx(refsolar,znew)
                  call solx(refsolar,znew,refsolchem) ! maj TD
               endif
            else if (izchange.eq.2) then
               write(*,220)
 220           format (/,2x,'enter new [Fe/H]')
               read (5,*) FeH
               call zFeH (2,znew,FeH,-1.d0)
            endif

            open (unit = 30, file = 'modini1.bin', form = 'unformatted',
     &           status = 'unknown') 
            call zchange (20,30,opt,solar)
     ...
     &           ' zinit.out'
      ...
      if (opt.eq.'2') then
c         iret=system("clear" )
         write (*,*) 'INITIAL MODEL PRODUCTION ',
     &        '(by homology from database)'
         write (*,150)
 150     format (/,2x,'enter stellar mass (in Mo)')
         read (5,*) mini
         write (*,210)
         read (5,*) zini
         call homol
      ...
      if (opt.eq.'3') then
c         iret=system("clear" )
         write (*,*) 'GENERATE INITIAL ROTATIONAL MODELS'
         write (*,250)
 250     format (/,2x,'enter rotational law : differential [d] or ',
     &        'solid [s]')
         read (5,300) rotationtype
 300     format(a1)
         if (rotationtype.ne.'d'.and.rotationtype.ne.'s') 
     &        stop 'wrong argument'
         write (*,350)
 350     format (2x,'enter rotational velocity (km/s)')
         read (5,*) diffvr
c         call binrot (diffvr,rotationtype)
         idiffvr = 6
         breaktime = 0.d0
      ...
      if (opt.eq.'4') then
c         iret=system("clear" )
         write (*,*) 'CONVERSION Z - [Fe/H]'
         call zFeH (0,zcalc,FeH,-1.d0)
         call mypause
         goto 88
      endif
...
c         iret=system("clear" )
      ...
      if (opt.eq.'6') then
c         iret=system("clear" )
         write (*,*) 'binary --> binary modification'
         call binbin (0)
         call mypause
         goto 88
      endif
...
c         iret=system("clear" )
      ...
      if (opt.eq.'a') then
c         iret=system("clear" )
         write (*,*) 'CONVERSION ROTATION VARIABLE : psi <-> theta'
         nout = 6
         call psitotheta
         call mypause
         goto 88
      ...
      if (opt.eq.'b') then
         ! Maj TD 07/2021
c         iret=system("clear" )
         write (*,*) 'CONVERSION SOLAR ABUNDANCES TO SOLAR MASS', 
     &        ' FRACTIONS'
 666     format (/,2x,'Choose set of reference chemical solar',
     &        ' abundances')
 400     write (*,401)
 401     format (/,5x,'Choose your option:',//
     ...
     &        3x,'o AY18, micro, rot, KS66 atm             [14]',//,
     &        '    ---> Enter option',5x,i2)
c     &        3x,'o Anders & Grevesse 1989            [AG89]',/,
c     &        3x,'o Grevesse & Noels 1993             [GN93]',/,
c     &        3x,'o Grevesse, Noesl & Sauval 1996     [GNS96]',/,
c     &        3x,'o Grevesse & Sauval 1998            [GS98]',/,
c     &        3x,'o Asplund, Grevesse & Sauval 2005   [AGS05]',/,
c     &        3x,'o Asplund, Grevesse, Sauval & Scott 2009 [AGSS09]',/,
c     &        3x,'o Asplund et al. 2009 + Ne from Young 2018 [AY18]',//,
c     &        '    ---> Enter option',5x,a6)
c         read (5,'(a6)') refsolar
c         print *,trim(refsolar)
          read (5,'(i2)') refsolchem
         print *,refsolchem
...
c         iret=system("clear" )   
     ...
     &        form = 'unformatted', status = 'old')
        open (unit = 30, file = 'modini1.bin', form = 'unformatted',
     &        status = 'unknown') 
         write (*,*) 'CHANGE INITIAL MASS FRACTION'
         call changexsp(20,30)
         call mypause
         goto 88
      endif

 222  print *,' *** modini.bin not found - program killed'
      stop
...
c..   Solid boby rotation
        print *,'Solid body rotation, omega =',omega_S
         do k = 1,nmod
            vomega(k) = omega_S
c            vomega(k) = -0.18d0*omega_S+dble(k)*0.5d0*omega_S/dble(nmod)
c            thetas(k) = 0.d0
c            xpsis(k) = 0.d0
            xpsis(k) = 0.d0
         enddo
...
c      vtheta(1) = vtheta(2)
      ...
      open (unit = 40, file = 'modang1.bin',form = 'unformatted',
     &     status = 'unknown')
...
c     &     (auxs(k),urs(k),vtheta(k),thetas(k),V_circ(k),
     ...
     &     ((xsp(k,l),l=1,nis-1),k=1,nmod)
      ...
      write (*,*)
      print *,'  input modini.bin,modang.bin -->', 
     &     'output modini1.bin, modang1.bin'
      write (*,*)
...
c      integer refsolchem ! maj TD
     ...
     &        "cd $DIR_INIT ; ls sini* | grep -v Z0 > /tmp/sini.dir")
      ...
      nfile = 0
!      do 10 i = 1,100    ! ne fonctionne pas, commenter TD 10/2020
      do 10 i = 1,64
...
c         print *,trim(refsolar)
     ...
     $            (refsolchem.eq.3.or.refsolchem.ge.5.and.
     $            refsolchem.le.11) then
             print *, 'AGSS09'
             refsolar='AGSS09'
      ! Grid   
          else if (refsolchem.eq.4) then
...
************************************************************************
      ...
      SUBROUTINE ZCHANGE (iu1,iu2,opt,solar)
...
c      integer refsolchem ! TD
      ...
      double precision xhe4,xo16,xc12,xna23,xli7,nLi,xh1,Liabund
...
! Commented TD 07/2021      
c$$$      if (refsolar.eq.'GN93') refsolchem = 1
c$$$      if (refsolar.eq.'AGS05') refsolchem = 2
c$$$      if (refsolar.eq.'AGSS09') refsolchem = 3
c$$$      if (refsolar.eq.'GRID') refsolchem = 4
c$$$  if (refsolar.eq.'AY18') refsolchem = 5
      if (refsolchem.eq.0) then
         print *, refsolchem, refsolar
         print *, "refsolchem = 0 malheureux !"
         stop
      endif
     
      write (31,99) refsolar
      print *,'solar reference composition :', refsolar, refsolchem
 99   format ('  o solar reference composition : ',a6)
      
      if (solar) then

         write (*,100) 
 100     format (/,2x,'introduce alpha enhancement (y/n) ?')
         read (5,200) calpha
         if (calpha.eq.'y') then
c     Field stars : [alpha/Fe] = +0.4 dex at low metallicity
c     47 Tuc : [alpha/Fe] = +0.3 dex (Carretta et al. 2004, A&A 416, 925)
            write (*,101) 
 101        format (/,2x,'introduce [alpha/Fe]')
            read (5,*) numalpha
            numalpha = 10.d0**numalpha
            write (*,102) log10(numalpha)
            write (31,102) log10(numalpha)
 102        format (' o alpha-elements enhanced by ',0pf6.3,' dex')
         else
            write (31,*) ' o alpha-elements NOT enhanced'
         endif
         write (*,150) 
 150     format (/,2x,'set deuterium abundance to zero (y/n) ?')
         read (5,200) deut
 200     format(a1)
         if (deut.eq.'y') then
            write (*,*) ' H2 abundance set to 1.d-30'
            write (31,*) 'o H2 abundance set to 1.d-30'
         endif
...
c$$$            h2h1 = 2.6d-5
c$$$            he3he4 = 1.04d-5*xini/yini
c     Coc&Vangioni.(2017 IJMPE..2641002C) : ! Modif TD. 30/06/2020
            h2h1 = 2.45d-5
            he3he4 = 1.07d-5*xini/yini
c     D/H = 2.45d-5
c     3He/H = 1.07d-5            
...
c     he3he4 = 5.1d-5*xini/yini
         ...
         if ((znew/x).lt.7.81245d-03) then
            testLi = 1
c            nLi = 4.14954d-10      ! li7/H - Coc et al 2004, ApJ
            nLi = 5.61d-10        ! li7/H - Coc & Vangioni 2017, JMPE
            xli7 = 7.0d0*x*nLi
            hxsp(7) = xli7
            write (*,111)
            write (31,111)
...
*----------------------------------------------------------
         ...
         xo16 = hxsp(io16)
         xna23 = hxsp(ina23)
         xli7 = hxsp(ili7)   ! modif Dec 19 
         if (opt.eq.'9') then
 400        write (*,401)
 401        format (/,5x,'Choose your option:',//
     &           3x,'o Change 4He fraction of mass          [1]',/,
     &           3x,'o Change [12C/Fe] abundance            [2]',/,
     &           3x,'o Change [16O/Fe] abundance            [3]',/,
     &           3x,'o Change [23Na/Fe] abundance           [4]',/,
     &           3x,'o Change 7Li abundance                 [5]',/,            
     &           3x,'o Exit                                 [6]',//,
     &           '    ---> Enter option',5x,a1)
            read (5,200) opt2
            
            if (opt2.ne.'1'.and.opt2.ne.'2'.and.opt2.ne.'3'.and.
     &           opt2.ne.'4'.and.opt2.ne.'5'.and.opt2.ne.'6') goto 400
            if (opt2.eq.'1') then
 409           write (*,410)
 410           format (5x,'Choose 4He fraction of mass:',5x,a1)
               read (5,*) xhe4
               if (xhe4.gt.1.d0-zreal) then
                  write (*,435)
                  goto 409
               endif
               goto 400
            ...
            if (opt2.eq.'2') then
 419           write (*,420)
 420           format (5x,'Choose [12C/Fe] abundance:',5x,a1)
               read (5,*) CFechoice
               xc12 = 10.d0**CFechoice*xspref(refsolchem,ic12)*xFenew
     &              /xFesol
               if (xc12.gt.hxsp(5)) then
                  write (*,440)
                  goto 419
               endif
               goto 400
            ...
            if (opt2.eq.'3') then
 429           write (*,430)
 430           format (5x,'Choose [16O/Fe] abundance:',5x,a1)
               read (5,*) OFechoice
               xo16 = 10.d0**OFechoice*xspref(refsolchem,io16)*xFenew
     &              /xFesol
               if (xo16.gt.hxsp(5)) then
                  write (*,440)
                  goto 429
               endif
               goto 400
            ...
            if (opt2.eq.'4') then
 431           write (*,432)
 432           format (5x,'Choose [23Na/Fe] abundance:',5x,a1)
               read (5,*) NaFechoice
               xna23 = 10.d0**NaFechoice*xspref(refsolchem,ina23)*
     &              xFenew/xFesol
               if (xna23.gt.hxsp(5)) then
                  write (*,440)
                  goto 431
               endif
               goto 400
            ...
            if (opt2.eq.'5') then
 433           write (*,432)
 434           format (5x,'Choose A(7Li) abundance:',5x,a1)
               read (5,*) Liabund
               xli7 = 10.d0**(Liabund-12.d0)*7.d0*hxsp(ih1)
               if (xli7.gt.hxsp(5)) then
                  write (*,440)
                  goto 433
               endif
               goto 400
 ...
 435        format (5x,'Invalid choice for 4He abundance!')
 440        format (5x,'Abundance is larger than chosen Z, choose new 
     &abundance!')
...
c..   else H is modified. In all cases the metallicity Z is modified
            ...
            if (opt2.eq.'6') then
               ...
               if (xli7.ne.hxsp(ili7)) then
                  write (*,1207) Liabund
                  write (31,1207) Liabund
               endif
               if (xhe4.eq.hxsp(ihe4)) then
                  hxsp(ihe4) = hxsp(ihe4)+hxsp(io16)-xo16+hxsp(ic12)
     $                 -xc12+hxsp(ina23)-xna23+hxsp(ili7)-xli7
                  hxsp(io16) = xo16
                  hxsp(ic12) = xc12
                  hxsp(ili7) = xli7
                  hxsp(ina23) = xna23
     ...
     &                 hxsp(ina23)-xna23+hxsp(ihe4)-xhe4+hxsp(ili7)-xli7
                  hxsp(ihe4) = xhe4
                  hxsp(ili7) = xli7
                  hxsp(io16) = xo16
                  hxsp(ic12) = xc12
                  ...
                  write (*,1206) xli7,hxsp(ili7)
                  write (31,1206) xli7,hxsp(ili7)
               endif
            endif
         endif
 ...
 1206    format (' o X(7Li) : ',1pe11.4,', --> ',1pe11.4)
 1207    format (' o A(Li)  imposed to ',0pf6.3)        
         ...
         if (calpha.eq.'y') write (*,102) log10(numalpha)
         FeH = log10(xFenew/(hxsp(2)*56.d0))-FeHsol
         ...
         write (*,8999) xsol,ysol,zsolar,log10(hxsp(7)/hxsp(2)/7.d0)
     $        +12.d0

      else
         rewind (iu1)
         call rmod(iu1)
         hxsp(1:nis-1) = xspref(refsolchem,1:nis-1)
         hxsp(7) = 10**(3.31d0-12.d0)*7*hxsp(2) ! modif TD fixation A(Li7) = 3.31
         hxsp(10) = 10**(1.41d0-12.d0)*9*hxsp(2)      ! modif TD fixation A(Be9) = 1.41
         hxsp(nis) = 1.d-50
         hxsp(nsp) = 1.d-50
	 zreal = 1.d0-hxsp(ih1)-hxsp(ih2)-hxsp(ihe3)-hxsp(ihe4)
         call wmod(iu1)
      endif
...
c.. second order accuracy in the spatial deriatives
     ...
     &     nmod),((vxsp(k,l),l = 1,nis-1),k = 1,nmod)
      ...
      do k = 1,nmod
         write(660,*),k,r(k),xpsis(k),omega(k)
      enddo
...
c      integer refsolchem  ! TD 07/2021 
...
c         refsolchem = 1 ! Modif TD
      endif
      if (refsolar.eq.'AGS05') then 
         fac = 1.0666D-03
c         refsolchem = 2  ! Modif TD
      endif
      if (refsolar.eq.'AGSS09') then 
         fac = 1.1953D-03
c         refsolchem = 3  ! Modif TD
      endif
      if (refsolar.eq.'GRID') then 
         fac = 1.147417671020920d-3
c         refsolchem = 4  ! Modif TD
      endif
      if (refsolar.eq.'AY18') then 
         fac = 1.1947208819D-03
c         refsolchem = 5  ! Modif TD
      endif
      if (refsolchem.eq.0) then
         print *, 'refsolchem = 0 malheureux !'
         stop
...
************************************************************************
      ...
      subroutine SOLX (initsolar,zinisol,refsolchem)
...
*      initsolar = 'AY18' => Asplund et al. 2009 + Ne from Young 2018  *      
************************************************************************
...
! Comment by TD 08/07/2021
c$$$      if (trim(initsolar).eq.'GN93') refsolchem = 1
c$$$      if (trim(initsolar).eq.'AGS05') refsolchem = 2
c$$$      if (trim(initsolar).eq.'AGSS09') refsolchem = 3
c$$$      if (trim(initsolar).eq.'GRID') refsolchem = 4
c$$$      if (trim(initsolar).eq.'AY18') refsolchem = 5
     ...
     $     trim(initsolar).eq.'AGSS09'.or.trim(initsolar).eq.'AY18')
     $     then
         read (10,1005)
      endif
      do l = 1,nelt
...
c$$$      else if (refsolchem.eq.3.or.refsolchem.eq.5) then
      else if (refsolchem.eq.3.or.refsolchem.le.5.and.refsolchem.le.14)   ! Maj TD 08/07/2021
     $        then   
c..   Asplund, Grevesse, Sauval & Scott 2009 (+ Young 2018)
...
* Isotopic ratios from Grevesse & Noels 1993 (see solar_abund file in
* this directory)

      data (abiso(l),l = 1,286) /
     &  .9999660, .0000340, .0001419, .9998581, .0749825, .9250175,   ! H1,H2,He3,He4,Li6,Li7
     & 1.0000000, .1988690, .8011310, .9890110, .0109890, .9963276,   ! Be9,B10,B11,C12,C13,N14
     &  .0036724, .9976158, .0003805, .0020037,1.0000000, .9297542,   ! N15,O16,O17,O18,F19,Ne20
     &  .0022576, .0679883,1.0000000, .7903075, .0997204, .1099720,   ! Ne21,Ne22,Na23,Mg24,Mg25,Mg26
     & 1.0000000, .9222767, .0467140, .0310093,1.0000000, .9501363,   ! Al27,Si28,Si29,Si30,P31,S32
     &  .0075001, .0421635, .0002001, .7580175, .2419825, .8413676,   ! S33,S34,S36,Cl35
