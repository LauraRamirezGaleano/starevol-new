      ...
      subroutine NULOSC (temp,den,abar,zbar,snu,dsnudt,dsnudd)
*                                                                      *
...
c..brem
     ...
     7                 gliq,gliqdt,gliqdd,gliqda,gliqdz 
      double precision z1,z2,sbrem1,sbrem2,sbremdt1,sbremdt2,sbremdd1,
     &     sbremdd2,factferm
...
c     fpairda = (xnumda - fpair*xdenda)*a1
c     fpairdz = (xnumdz - fpair*xdendz)*a1
...
c     xdendz = d*rmdz*c 
      ...
      qpairdd = xnum*xdendd
c     qpairda = xnum*xdenda
c     qpairdz = xnum*xdendz
...
c..equation 2.5
      ...
      spairdd = a1*fpairdd
c     spairda = a1*fpairda
c     spairdz = a1*fpairdz

      a1      = spair
      spair   = gl*a1
      spairdt = gl*spairdt + gldt*a1
      spairdd = gl*spairdd
c     spairda = gl*spairda
c     spairdz = gl*spairdz
...
c     spairda = a1*spairda + a2*qpairda*a3
c     spairdz = a1*spairdz + a2*qpairdz*a3
...
c     gl2da = 1.1095d11 * (rmda*c00  - d*rmda)
c     gl2dz = 1.1095d11 * (rmdz*c00  - d*rmdz)
...
c       dumda = xdenda + 1.25d0*xnumda
c       dumdz = xdendz + 1.25d0*xnumdz
       end if
...
c      c03 = f1*(dumda + a3*0.25d0*xnumda)
c      c04 = f1*(dumdz + a3*0.25d0*xnumdz)

       fxy   = 1.05d0 + (a1 - b1)*c00
       fxydt = (a2*xnumdt -  b2*xnumdt)*c00 + (a1-b1)*c01
       fxydd = (a2*xnumdd -  b2*xnumdd)*c00 + (a1-b1)*c02
c      fxyda = (a2*xnumda -  b2*xnumda)*c00 + (a1-b1)*c03
c      fxydz = (a2*xnumdz -  b2*xnumdz)*c00 + (a1-b1)*c04
...
c     splasda = (ftda + flda)*fxy + (ft+fl)*fxyda
c     splasdz = (ftdz + fldz)*fxy + (ft+fl)*fxydz
...
c     splasda = a2*splasda + a3*gl2da*a1
c     splasdz = a2*splasdz + a3*gl2dz*a1
...
c     splasdz = a2*splasdz + a3*gl2dz*a1
      ...
      splasdd = a2*splasdd 
c     splasda = a2*splasda 
c     splasdz = a2*splasdz 
...
c     dumda = a1*zetada + 2.0d0*a2*zeta*zetada
c     dumdz = a1*zetadz + 2.0d0*a2*zeta*zetadz
...
c     xnumda = dumda*z - dum*z*cc*zetada
c     xnumdz = dumdz*z - dum*z*cc*zetadz
...
c     fphotda = (xnumda - fphot*xdenda)*dum
c     fphotdz = (xnumdz - fphot*xdendz)*dum
...
c     xdendz =  rmdz*z
      ...
      dum      = -qphot*z
      qphotdd = dum*xdendd
c     qphotda = dum*xdenda
c     qphotdz = dum*xdendz

c..equation 3.2
      sphot   = xl5 * fphot
      sphotdt = 5.0d0*xl4*xldt*fphot + xl5*fphotdt
      sphotdd = xl5*fphotdd
c     sphotda = xl5*fphotda
c     sphotdz = xl5*fphotdz

      a1      = sphot
      sphot   = rm*a1
      sphotdt = rm*sphotdt  
      sphotdd = rm*sphotdd + rmdd*a1  
c     sphotda = rm*sphotda + rmda*a1  
c     sphotdz = rm*sphotdz + rmdz*a1  
...
c     sphotda = a1*sphotda + a2*qphotda*a3
c     sphotdz = a1*sphotdz + a2*qphotdz*a3

      if (sphot .le. 0.d0) then
       sphot   = 0.0d0
       sphotdt = 0.0d0
       sphotdd = 0.0d0
c      sphotda = 0.0d0
...
c      if (temp .gt. 0.3d0 * tfermi) then         
      if (temp .ge. (0.3d0-factferm) * tfermi) then         
...
c      xdenda = (c03 - xden*dumda)*z
c      xdendz = (c04 - xden*dumdz)*z
...
c      gbremda = xnumda + xdenda
c      gbremdz = xnumdz + xdendz
...
c      dumda  = -dum*abari
c      dumdz  = 0.5738d0*2.0d0*ye*t86*den

       z1       = tfac4*fbrem - tfac5*gbrem
       sbrem1   = dum * z1
       sbremdt1 = dumdt*z1 + dum*(tfac4*fbremdt - tfac5*gbremdt) 
       sbremdd1 = dumdd*z1 + dum*(tfac4*fbremdd - tfac5*gbremdd) 
c      sbremda  = dumda*z1 + dum*(tfac4*fbremda - tfac5*gbremda) 
c      sbremdz  = dumdz*z1 + dum*(tfac4*fbremdz - tfac5*gbremdz) 
...
c      else
      if (temp .le. (0.3d0+factferm) * tfermi) then         
...
c      dumda = -oneth*dum*abari
c      dumdz = 2.0d0*dum*zbari
...
c      fliqda = a0*dumda*(fb - ft)
c      fliqdz = a0*dumdz*(fb - ft)

       gliq   = w*gb + (1.0d0 - w)*gt
       gliqdt = a1*dumdt*(gb - gt)
       gliqdd = a1*dumdd*(gb - gt) + w*c02 + (1.0d0 - w)*c03
c      gliqda = a1*dumda*(gb - gt)
c      gliqdz = a1*dumdz*(gb - gt)
...
c      dumdz  = 0.5738d0*2.0d0*ye*t86*den
       ...
       z2       = tfac4*fliq - tfac5*gliq
       sbrem2   = dum * z2
       sbremdt2 = dumdt*z2 + dum*(tfac4*fliqdt - tfac5*gliqdt) 
       sbremdd2 = dumdd*z2 + dum*(tfac4*fliqdd - tfac5*gliqdd) 
c      sbremda  = dumda*z2 + dum*(tfac4*fliqda - tfac5*gliqda) 
c      sbremdz  = dumdz*z2 + dum*(tfac4*fliqdz - tfac5*gliqdz) 

      end if


      if (temp .gt. (0.3d0+factferm) * tfermi) then         
         z = z1
         sbrem = sbrem1      
         sbremdt = sbremdt1
         sbremdd = sbremdd1
      else if (temp .le. (0.3d0-factferm)* tfermi) then
         z = z2
         sbrem = sbrem2      
         sbremdt = sbremdt2
         sbremdd = sbremdd2
      else
         dum = (temp-(0.3d0-factferm)*tfermi)/(2.d0*factferm*tfermi)
         z = z1*dum + z2*(1.d0-dum)
         sbrem = sbrem1*dum + sbrem2*(1.d0-dum)
c         z = exp(log(z1)*dum + log(z2)*(1.d0-dum))
c         sbrem = exp(log(sbrem1)*dum + log(sbrem2)*(1.d0-dum))
         sbremdt = sbremdt1*dum + sbremdt2*(1.d0-dum)
         sbremdd = sbremdd1*dum + sbremdd2*(1.d0-dum)
      endif
...
c..table 12
      if (nu .ge. -20.d0  .and. nu .lt. 0.d0) then
       a1 = 1.51d-2
       a2 = 2.42d-1
       a3 = 1.21d0
       b  = 3.71d-2
       ...
       f2 = 0.0d0
       f3 = 0.0d0
      else if (nu .ge. 0.d0  .and. nu .le. 10.d0) then
       a1 = 1.23d-2
       a2 = 2.66d-1
       a3 = 1.30d0
       b  = 1.17d-1
...
c..equation 6.7, 6.13 and 6.14
      if (nu .ge. -20.d0  .and.  nu .le. 10.d0) then

       zeta   = 1.579d5*zbar*zbar*tempi
       zetadt = -zeta*tempi
       zetadd = 0.0d0
c      zetada = 0.0d0
c      zetadz = 2.0d0*zeta*zbari
       ...
       dumdd  = zeta*c01*nudd
c      dumda  = zeta*c01*nuda
c      dumdz  = zetadz*c00 + zeta*c01*nudz
...
c      gumda  = dd00*c*nuda + b*z*d*dumda  
c      gumdz  = dd00*c*nudz + b*z*d*dumdz  
...
c      bigjda = c01*dumda*z*a1 + c00*z*nuda*a1 - c00*z*a1*a1 * gumda
c      bigjdz = c01*dumdz*z*a1 + c00*z*nudz*a1 - c00*z*a1*a1 * gumdz
...
c      srecoda = sreco*(-1.0d0*abari + bigjda*a2 - z*(zetada+nuda)*a1)
c      srecodz = sreco*(14.0d0*zbari + bigjdz*a2 - z*(zetadz+nudz)*a1)
...
c     spairda = spairda*deni
c     spairdz = spairdz*deni  

      splas   = splas*deni
      splasdt = splasdt*deni
      splasdd = splasdd*deni - splas*deni
c     splasda = splasda*deni
c     splasdz = splasdz*deni  

      sphot   = sphot*deni
      sphotdt = sphotdt*deni
      sphotdd = sphotdd*deni - sphot*deni
c     sphotda = sphotda*deni
c     sphotdz = sphotdz*deni  

      sbrem   = sbrem*deni
      sbremdt = sbremdt*deni
      sbremdd = sbremdd*deni - sbrem*deni
c     sbremda = sbremda*deni
c     sbremdz = sbremdz*deni  

      sreco   = sreco*deni
      srecodt = srecodt*deni
      srecodd = srecodd*deni - sreco*deni
c     srecoda = srecoda*deni
c     srecodz = srecodz*deni  
...
c     dsnuda =  splasda + spairda + sphotda + sbremda + srecoda 
c     dsnudz =  splasdz + spairdz + sphotdz + sbremdz + srecodz 
