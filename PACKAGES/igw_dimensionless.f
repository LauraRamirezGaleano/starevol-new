C L. Ramirez (Feb 2025) with Equatorial Plane Model, simplify case m = 1 in tau
...
C Auteur : S.Talon
C
C Modification L.Ramirez (2025) with Equatorial Plane Model, Correction Rotation. 
      ...
      double precision amp_crit, abslogamp_crit
      double precision alpha_val, numerator, denominator, coriolisFactor
      double precision Fr, S, Y, baseFactor, drFactor
      integer k, k1, un, allerretour
      logical retrograde
      
      !LR Debugging 
      print *, "LR Entering OND_INT subroutine"
      ! print *, "mtu=", mtu, "npasr=", npasr, "ncouche=", ncouche
      ! print *, "ll=", ll, "mm=", mm, "omegaloc=", omegaloc
      ...
      ! Compute the retrograde wave amplitude
      do while (retrograde)
         k = k1
         k1 = k + un
         ...
         if (sigmaret < 1.d-16) sigmaret = 1.d-16

         ! Buoyancy factor
         xN2 = dsqrt(abs(xN_o(k) / (xN_o(k) - sigmaret**2)))
         !print *, "xN2=", xN2 , "Should be positive" !LR Debugging 

         ! Compute Fr, S, and Y
         if (xN_o(k) > 0.d0) then
            Fr = sigmaret/dsqrt(xN_o(k))
            S = S_number(k)
            Y = Y_number(k)
         else
            Fr = 0.d0
         ...
         ! Compute baseFactor as in damping
         if (Fr .gt. 0.d0) then
            baseFactor = (ll1**1.5d0 * (xKm_o(k) + xnuvv_o(k)))/ 
     &                   (brunt_o(k) * (Fr**4))
         else
            baseFactor = 0.d0
         endif
         ...
         alpha_val = 1.d0 - (1.d0 / ll1)
         numerator = 1.d0 + alpha_val * S * (S + Y)
         denominator = dsqrt(1.d0 - Fr**2.d0 + numerator)
         coriolisFactor = numerator / denominator

         ! Compute the integrand for optical depth
         !baseFactor = ll1 * fint(k) * xN2 / sigmaret**4
         drFactor = (1.d0 / hnenn_o(k)) / (rraym_o(k)**3 * rtot**2)
         dintret = baseFactor * coriolisFactor * drFactor

         ! Accumulate optical depth
         xintret(k1) = xintret(k) + dintret

         ! Check if the wave is fully damped
         if (xintret(k1) > abslogamp_crit) then
            ampret(k1) = 0.d0
            retrograde = .false.
         else
         ...
         !print *, "Layer:", k
         !print *, "Fr =", Fr, "S =", S, "Y =", Y
         !print *, "LR Coriolis Factor =", coriolisFactor
         !print *, "Optical Depth Increment (dintret) =", dintret
         !--

         ! Update local momentum deposition
         depotonde(k1) = depotonde(k1) - ampret(k) + ampret(k1)

         ! Check for boundary conditions
         if ((un == 1 .and. sigmaret > brunt_o(k + un + un)) .or.
     &       k1 == npasr .or. rray_o(k) < 1.d-3) then
            un = -1
         else if (k1 == mtu) then
            un = +1
            retrograde = .false.
      ...
      print *, "LR Exiting OND_INT subroutine" !LR Debugging

      return
      end subroutine ond_int
...
C      if(iterondes.eq.1) then 
      if (igwsurfrot) then 
          do ii=1,nfreq
             do ll=1,llmax
                do mm=1,ll
                   print *,'LR xlum_ond(ii,ll,mm)=',xlum_ond(ii,ll,mm),
     &             'should be > 0'
                                       
                   if (xlum_ond(ii,ll,mm).gt.0.d0) then
                      !print *,"LR OND_INT subroutine for ll=",ll,
     &                !         "mm=",mm," ii=",ii
                      call ond_int(mtu,npasr,ncouche,ll,mm,freqonde(ii)
     &                     ,d_om)
                      !print *, "LR Returned from OND_INT subroutine"
C               ----
...
!     ----
      ...
      !print *, "LR Calling DAMPING subroutine"
      if (igwsurfrot) call damping(mtu,npasr,ncouche,xmoment_tot)
      !print *, "Returned from DAMPING subroutine"
!     ----
C Modif CC (30/10/09)
      if (igwcorerot.and.(npasr.ne.ncouche)) 
...
! Computes the damping(tau) of internal gravity waves (IGWs)
! propagating through the stellar radiative zone, including Coriolis
! corrections. Here tau is analogous to an optical depth: wave amplitudes
! decay as exp(-tau). Filters out non-propagating waves and accumulates
! energy and momentum fluxes. 
! Last Modification by L.Ramirez Galeano 10-May-2025.
C ---------------------------------------------------------------------
...
!Input arguments 
      integer mtu,npasr,ncouche
      double precision xmoment_tot
!Local variables      
      integer i,ii,ll,mm,k,k1,j,jj
      double precision e_total
c      double precision amplitudemoyenne,amplitudemoyenne_core
      
      double precision tau,kr,ll1,e_tot,e_freq,xN2,domega
...
C Test CC (31/10/09)
      ...
      !Modif LR 20250214
      !double precision Ri_number(nsh)
      !double precision S_number(nsh)
      !double precision Y_number(nsh) 
      !double precision Fr_number(nsh,nfreq)
      double precision baseFactor(nsh)
      double precision Y,S_n,Fr
      double precision alpha_val,numerator(nsh),denominator(nsh)
      double precision coriolfactor(nsh),drfactor(nsh)
      data npi/6.2831853d-6/
...
c     &           amplitudemoyenne_core(-2:2,nfreq,llmax)
      !LR Modif 20250214
      print *, "Entering DAMPING subroutine"
      print *, "mtu=", mtu, "npasr=", npasr, "ncouche=", ncouche
C
C -------------------------------------------------------------------
C Recherche de la frequence de Brunt-Vaisala 0.5Hp au dessous de la ZC
C Servira de valeur minimale a N
! 1) Fix N below the base of convective envelope to avoid N->0 singularity
...
C ---- Modif LR 20250515
C Definition of the dimensionless numbers
      print *, 'vomega in damping', vomega
      call compute_dimensionless_numbers(nmod, r)!, vomega) 
      print *, "damping Dimensionless numbers computed"
...
C Modif CC (5/4/7)
! Determine starting layer for damping integration (5% below mtu)
...
C          xN2=dsqrt(abs(xN_o(i)/(xN_o(i)-freqrefinert(ii)**2)))
          ...
          dintpro=ll1*fint(k)*xN2/freqrefinert(ii)**4 !LR NEED TO BE MODIFIED 
          xint=xint+dintpro
	  ampli(k)=exp(-xint)
	  if (xint.gt.100) then
	    go to 100
...
C Ici, on va calculer le flux de moment en tenant compte du profil de rotation
     ...
     &         ampp,ampm)
      ...
      print *, "Exiting DAMPING subroutine"
      !print *, "Total energy flux in waves (e_tot) =", e_tot
...
C            print *,"A: elimination de nu=",freqrefinert(ii),"l=",ll
     ...
     &                 .and.i.gt.mtu)
              xN2=dsqrt(abs(xN_o(i)/(xN_o(i)-freqrefinert(ii)**2)))
              tau=tau+ll1**1.5d0*fint(i)*xN2/freqrefinert(ii)**4 !LR NEED TO BE MODIFIED
              i=i-1
            enddo
...
C     &     sigmaret**2,xN2
           ...
           dintret=ll1*fint(k)*xN2/sigmaret**4 !LR NEED TO BE MODIFIED
           xintret(k1)=xintret(k)+dintret 
...
C        xN2=dsqrt(xN_o(k)/(xN_o(k)-sigmapro**2))
        ...
        dintpro=ll1*fint(k)*xN2/sigmapro**4 !LR NEED TO BE MODIFIED
...
C LR Modif 20250515
c ---------------------------------------------------------------------
      subroutine compute_dimensionless_numbers(nmod, r)!, vomega)
      implicit none
      include 'evolpar.star'
      include 'evolcom.igw'
      include 'evolcom.rot'
      ...
      ! Input arguments
      integer :: nmod
      double precision :: r(nmod)!, vomega(nmod)

      ! Local variables
      integer :: i, ii
      double precision :: tamp
      print *, "Dimensionless numbers computed:"

      ! Compute Ri, S, Y for each shell
      do i = 1, nmod-1
...
c          Ri(i) = 0.d0
          ...
          !tamp = (vomega(i+1) - vomega(i)) / (r(i+1) - r(i))
           print *,'vomega in dimensionless=',vomega(i+1),vomega(i) 
           print *, "tamp = ",tamp, "vomega(i+1)-vomega(i) = ", 
     &     vomega(i+1)-vomega(i),"r(i) = ", r(i), "r(i+1) = ", r(i+1)
!          ! Richardson number: Ri = N^2 / (r·50·dΩ/dr)^2
!          if (tamp /= 0.d0) then
!             print *, "tamp != 0"
!             Ri_number(i) = xN_o(i) /((r(i)*5.d1*tamp)**2)
!          else
!             Ri_number(i) = 0.d0
!          endif
      !    if (Ri_number(i) <= 0.d0 .or. Ri_number(i) > 1.d30) then
      !       Ri_number(i) = 0.d0
      !    endif
  
         ! Y number: Y = Ri^(-1/2)
         if (Ri_number(i) > 0.d0) then
            Y_number(i) = 1.d0 / dsqrt(Ri_number(i))
         else
            Y_number(i) = 0.d0
         endif

         ! S number: S = 2 Omega / N
         if (xN_o(i)>0.d0) then
            S_number(i) = 2.d0 * vomega(i) / dsqrt(xN_o(i))
         else
            S_number(i) = 0.d0
         endif

      !    print *, "Shell:", i
         print *, "Ri_number(i) =", Ri_number(i)
      !    print *, "S_number(i) =", S_number(i)
      !    print *, "Y_number(i) =", Y_number(i)

      enddo

      ! Set the last shell values
      Ri_number(nmod) = Ri_number(nmod-1)
      Y_number(nmod) = Y_number(nmod-1)
      S_number(nmod) = S_number(nmod-1)


      ! Compute Fr for each frequency and shell
      do i = 1, nmod-1
         do ii = 1, nfreq
            if (xN_o(i) > 0.d0) then
         ...
         enddo
      enddo
      Fr_number(nmod, :) = Fr_number(nmod-1, :)

      return
      end subroutine compute_dimensionless_numbers
