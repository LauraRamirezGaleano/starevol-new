...
C Test CC (31/10/09)
      ...
      double precision kr_mode(nfreq,llmax,2*llmax + 1)  !LR Modif 20250325
      double precision tot_tau,tot_kr !LR Modif 20250325 
      data npi/6.2831853d-6/
...
c      stop "xN_o"
      e_tot=0.d0
      ! Debug print header
      !print *, "=== Debug output for damping and kr calculation ==="
      do ii=1,nfreq
...
c           print *,"damping",ii,ll,xN_o(mtu+1),freqrefinert(ii)**2
          !tau=0.d0 LR commented 20250325
          tot_tau = 0.d0                    ! Initialize total tau for this l
          tot_kr = 0.d0                    ! Initialize total kr for this l
          ll1=(dfloat(ll)+1.d0)*ll
          !Loop over m modes (mode-by-mode resolution) LR Modif 20250325
          !print *,"ll:",ll
          do mm=-ll,ll
            !print *,"m:",mm
            m_index=mm+ll+1 ! Map m from -ll...ll to 1...2*ll+1
            tau=0.d0
            i=mtu+1
            !Check if the wave can propagate
            !print *,"LR xN_o(i)",xN_o(i)
            !print *,"LR freq**2",freqrefinert(ii)**2
            if (xN_o(i) .le. freqrefinert(ii)**2) then
              !Wave cannot propagate
              tau_mode(ii,ll,m_index) = 0.d0
              kr_mode(ii,ll,m_index) = 0.d0
              !print *,"m:",mm,"m_index:",m_index
              print *,"Wave cannot propagate, tau=0"
              !print *,"A: elimination de nu=",freqrefinert(ii),"l=",ll
            else

              !Wave can propagate
              !Loop over the layers
                  !print *,"Wave can propagate"
                  !print *,"m:",mm,"m_index:",m_index,"l=",ll
                  
                  !print *,"m:",mm,"alpha_val:",alpha_val
                  !check each condition separately:
                  ! if ((tau.lt.0.5d0)) then
                  !       print *,"[DEBUG 1] check"
                  ! endif
                  
                  ! if ((xN_o(i).gt.freqrefinert(ii)**2)) then
                  !       print *,"[DEBUG 2] check"
                  ! endif
                  ...
                  ! if ((i.lt.ncouche-2)) then
                  !       print *,"[DEBUG 4] check"
                  ! endif
                  ! print *,"Wave can propagate"
                  do while ((tau.lt.0.5d0).and.(xN_o(i).gt.
     &                   freqrefinert(ii)**2)
     &                 .and.(i.lt.npasr).and.(i.lt.ncouche-2))
                        ...
                        alpha_val= 1.0d0-(mm**2/ll1) !To be changed (1-m2/h2) h2 = ll1
     ...
     &                        xnuvv_o(i)))/(brunt_o(i) * (Fr**4))
                        ...
                        denominator(i)=dsqrt(1.d0-Fr**2.0d0+
     &                   numerator(i))
                        coriolFactor(i)=numerator(i)/denominator(i)

                        ! 4) Geometric factor: dr/r^3
                        drFactor(i)=(1.d0/hnenn_o(i))/(rraym_o(i)**3
     &                   *rtot**2)
                        ! 5) Update tau for this (l,m) mode
                        tau=tau+baseFactor(i)*coriolFactor(i)*
     &                   drFactor(i)
                        !print *, "DEBUG: Fr =",Fr
                        i=i+1
                  enddo
                  tau_mode(ii,ll,m_index) = tau
                  kr_mode(ii,ll,m_index) = 1.0d0/Fr*dsqrt(1-Fr**2+
     &             alpha_val*S_n*(S_n+Y))*dsqrt(ll1)/(rray_o(i)*rtot)
            endif
            tot_tau = tot_tau + tau_mode(ii,ll,m_index)
            tot_kr = tot_kr + kr_mode(ii,ll,m_index)
          enddo ! End loop over m modes

          ! Use the integrated tau for degree ll by summing m contributions.
            fluxtotalafiltrer(ii, ll) = tot_tau
...
!      &         *dsqrt(ll1)/(rray_o(i)*rtot)
            kr = tot_kr/(2.0d0*ll+1.0d0)
            !print *, "Int tau for l =",ll,":",fluxtotalafiltrer(ii, ll)
            !print *, "Debug kr", kr

            domega=rray_o(mtu)-rray_o(i)
            if (domega.lt.(1.d0/(kr*rtot)).or.i.eq.(mtu+2)) then
               fluxtotalafiltrer(ii,ll)=0.d0
               print *, "domega, fluxtotalafiltrer(",ii,",",ll, ") to 0"
c               print *,"C: elimination de nu=",freqrefinert(ii),"l=",ll
     ...
     &           *(2.d0*ll+1.d0)
        ...
        e_tot=e_tot+e_freq
      enddo   ! Boucle sur les frequences
      print *, "Final integrated energy flux e_tot =", e_tot
