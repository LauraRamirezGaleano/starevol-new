...
c      include 'evolcom.transpondesexcit'
      ...
      double precision integrand,numerator(nsh)
      !double precision alpha_val,dr,N,N2, Omega_d
      double precision alpha_val, Omega_d(nsh)
      !double precision tamp,zeta,ll1_15,coriolis
      double precision tamp(nsh),zeta(nsh),ll1_15,coriolis(nsh)
      ...
      ! integer unit_wave
      ! common /waveunit/ unit_wave
      ! integer, parameter :: l_target = 2, m_target = 1
      ! double precision, parameter :: freq_target = 1.d0 !microHz!1.0d-6  ! Example value (rad/s)?
      ...
      ll1_15 = (dfloat(ll+1)*dfloat(ll))
      !ll1_15 = ll1**1.5d0
      alpha_val = 1.d0 - dfloat(mm*mm)/ll1_15!dfloat(mm*mm)/ll1
      do k=1,ncouche
        xintret(k)=0.d0
        depotonde(k)=0.d0
      enddo

! Precompute dr and domega/dr arrays 
      do k = 1, ncouche-1
         dr_arr(k) = r(k+1) - r(k)
...
C Including Coriolis term LR (29/05/2025)
C ----------------------------------------------------------
      k1=mtu
      ampret(k1)=1.d0
      un=1
...
C ------------------------------------------
      do while (retrograde)
        ...
        Omega_d(k) = vomega(k)
        tamp(k) = domega_arr(k)/dr_arr(k)
        !dr = dr_arr(k)
        
        !zeta = 2.d0*Omega_d +(r(k)*tamp)
        zeta(k) = 2.d0*Omega_d(k) +(r(k)*tamp(k))
        !coriolis = (alpha_val*2.d0*Omega_d*zeta)
        coriolis(k) = (alpha_val*2.d0*Omega_d(k)*zeta(k))

        !coriolis = 0.0d0
!         if (domega_arr(k).gt.0.d0) then
! !             print *, 'domega_arr(',k,')=',domega_arr(k),'zeta=',zeta,
! !      &      'Omega_d=',Omega_d
!             print *, 'domega_arr(',k,')=',domega_arr(k),'zeta='
!      &      ,zeta(k),'Omega_d=',Omega_d(k)
!             print *, 'l, m=', ll, mm
!         endif 

        !numerator(k) = xNt_o(k)+coriolis
        numerator(k) = xNt_o(k)+coriolis(k)


        !xN2=dsqrt(abs(xN_o(k)/(xN_o(k)+coriolis-sigmaret**2)))
        xN2=dsqrt(abs(xN_o(k)/(xN_o(k)+coriolis(k)-sigmaret**2)))

        dintret=ll1*(fint(k)/xNt_o(k))*xN2*numerator(k)/sigmaret**4            

        xintret(k1)=xintret(k)+dintret
        !print *, 'k=',k,' dintret=',dintret
c dintret < 0
...
!      &       abs(omegaloc - freq_target) < 1d-10) then
!              write(unit_wave,*) k1,r(k1),ampret(k1),
!      &        xintret(k1),depotonde(k1)
!          endif
...
!       if (first_call) then
!          unit_wave = 99
!          open(unit=unit_wave, file='wave_evolution.dat',
!      &     status='UNKNOWN')
!          first_call = .false.
!       endif
      !-------------------
...
C Test CC (31/10/09)
      ...
      double precision alpha_val, numerator(nsh)
      double precision Omega_d,coriolis(nsh)
      double precision tamp, zeta
      data npi/6.2831853d-6/
...
C            print *,"A: elimination de nu=",freqrefinert(ii),"l=",ll
     ...
     &                 .and.i.lt.npasr.and.i.lt.ncouche-2)
            ...
            ! Compute Omega and dOmega/dr
              ...
              zeta = (2.d0*Omega_d) + (r(i)*tamp)
              coriolis = alpha_val*2.d0*Omega_d*zeta
              numerator = xNt_o(i) + coriolis

              xN2=dsqrt(abs(xN_o(i)/(xN_o(i)+
     &        coriolis(i)-freqrefinert(ii)**2)))
                  
              tau=tau+ll1**1.5d0*(fint(i)/xNt_o(i))*xN2*numerator(i)/
     &            freqrefinert(ii)**4
              !tau=tau+ll1**1.5d0*fint(i)*xN2/freqrefinert(ii)**4 !xNt_o in fint 

              i=i+1
            enddo
            kr=dsqrt(((xN_o(i-1)+2.d0*Omega_d*zeta)
...
!           numerator = xNt_o(k) + coriolis

!           print *,'coriolis', coriolis
!           xN2=dsqrt(abs(xN_o(k)/
!      &        (xN_o(k)+coriolis-freqrefinert(ii)**2)))
          xN2=dsqrt(abs(xN_o(k)/(xN_o(k)-freqrefinert(ii)**2)))
          dintpro=ll1*fint(k)*xN2/freqrefinert(ii)**4
!           dintpro=ll1**1.5d0*(fint(k)/xNt_o(k))*xN2*numerator(k)
!      &            /freqrefinert(ii)**4
          xint=xint+dintpro
      !     print *,"LR LUMWAVE",'dintpro', dintpro
	  ampli(k)=exp(-xint)
	  if (xint.gt.100) then
	    go to 100
	  endif
